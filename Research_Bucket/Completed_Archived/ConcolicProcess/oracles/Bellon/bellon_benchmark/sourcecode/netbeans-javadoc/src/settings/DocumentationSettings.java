...
public class DocumentationSettings extends SystemOption {
    ...
    public ServiceType getServiceTypeOfDocument() {
        JavadocSearchType.Handle st = (JavadocSearchType.Handle)getProperty( PROP_SEARCH );
        JavadocSearchType t = null;
	int x = 15;
        if (st != null) {
	    x++;
            t = (JavadocSearchType)st.getServiceType(); }
        if (t == null) {
	    --x;
            if (isWriteExternal()) {
		x = x > 15 ? 15 : x;
