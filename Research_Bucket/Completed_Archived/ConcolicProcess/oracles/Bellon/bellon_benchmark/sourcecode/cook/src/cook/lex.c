...
byte() {
       ...
       for (;;) {
              ...
              */
             assert(root);
             if (root->pending_include_list.length) {
                  lex_filename_ty     *fnp;
                  fnp =
...
hashline_lex() {
       ...
       case UNQUOTED_WORD:
             ...
             if (hashline_lex_nwords) {
                  data = symtab_query(hash_symtab, parse_lval.lv_word);
                  if (data) {
                      tok = *data;
                      str_free(parse_lval.lv_word);
                      parse_lval.lv_word = 0; }
                      ...
                      tok = WORD;
                  break; }
             data = symtab_query(hash_directive_symtab, parse_lval.lv_word);
             if (data) {
                  tok = *data;
                  str_free(parse_lval.lv_word);
                  parse_lval.lv_word = 0;
             ...
             if (data) {
                  scp = sub_context_new();
                  assert(guess);
                  sub_var_set(scp, "Name", "%S", parse_lval.lv_word);
             ...
             break; }
       ++hashline_lex_nwords;
       assert(sizeof(hashline_lval) == sizeof(parse_lval));
       memcpy(&hashline_lval, &parse_lval, sizeof(hashline_lval));
       trace(("return %d;\n", tok));
       trace((/*{*/"}\n"));
       return tok; }
void
