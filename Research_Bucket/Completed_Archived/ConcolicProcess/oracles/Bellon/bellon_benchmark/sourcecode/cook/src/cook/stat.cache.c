 ...
 */
static void stat_cache_fingerprint _((string_ty *, cache_ty *));
static void
stat_cache_fingerprint(path, cp)
       ...
       if (fp) {
             ...
             if (fp->newest == cp->newest) {
                   ...
                   */
                  cp->oldest = fp->oldest; }
             else if (cp->newest < fp->newest) {
                  string_ty   *s;
                  /*
                   * it travelled backwards in time
                   *  (e.g. ClearCase or 3DFS or unionfs, etc)
                   * since we last saw it.  (Remember the
                   * fingerprint, but it plays no part in the
                   * comaprison.)
                   */
