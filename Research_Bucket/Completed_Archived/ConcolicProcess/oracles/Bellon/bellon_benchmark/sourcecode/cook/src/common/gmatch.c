...
static void       *erf_aux;
void
gmatch_error_handler(fp, aux)
       gmatch_fp     fp;
       void   *aux; {
       erf = fp;
       erf_aux = aux; }
static void report_error _((const char *));
static void
report_error(s)
       const char    *s; {
       sub_context_ty        *scp;
       ...
       erf(erf_aux, message);
       str_free(message); }
static void missing_closing_bracket _((void));
static void
 ...
 */
static int gmatch_inner _((const char *, const char *, const char *,
       const char *));
static int
gmatch_inner(formal, formal_end, actual, actual_end)
       const char    *formal;
       const char    *formal_end;
       const char    *actual;
       const char    *actual_end; {
       const char    *cp;
       int     result;
       trace(("gmatch_inner(formal = %8.8lX, formal_end = %8.8lX, \
actual = %8.8lX, actual_end = %8.8lX)\n{\n", (long)formal, (long)formal_end,
(long)actual, (long)actual_end));
       ...
       return result; }
int
gmatch2(formal, formal_end, actual)
       const char    *formal;
       const char    *formal_end;
       const char    *actual; {
       original = formal;
