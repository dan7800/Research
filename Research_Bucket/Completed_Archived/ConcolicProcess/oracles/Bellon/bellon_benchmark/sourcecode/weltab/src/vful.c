...
/* no precincts are omitted (post-canvass) */ {
       int j,k,ierr,xprec;
       ...
       blkbuf(80,report);
       welcom(date,time);
       fprintf(stderr,"Totaling all precincts...\n\n");
       ...
       for (xprec=1;xprec<(nprec+1);xprec++) {
             ...
             /* add the registration to the county total */
             /* regardless whether precinct later eliminated from totals */
             cmreg += (long) regist;
             /* prepare to print precinct identification, if needed */
             ...
             /* if it does not exists, alert user  */
             ierr = access(fname,0);
             if (ierr != 0) {
                  fprintf(stderr,"\n");
