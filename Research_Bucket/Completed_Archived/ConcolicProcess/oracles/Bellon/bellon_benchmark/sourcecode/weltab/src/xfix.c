...
#include "unitinfo.h"
main()
/* correct major mistakes in prec. results caused by changing tables */
/* version of VEDT without eliminating races precinct cannot vote for */ {
...
x499:  noop();
       ...
       for (j=1;j<(noffic+1);j++) {
             ...
             if (! lallow(j,vtprec,indxpr,vtunit)) {
                  fprintf(stderr,
                  "This office (%d) is NOT ALLOWED for this precinct (%ld).\n",
                      j,vtprec);
                  };
             /* print office name */
x525:   sclear();
