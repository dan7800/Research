...
#include "unitinfo.h"
main()
       /* query the user for precinct results AFTER Speed-Entry (SPED) */
       /* version of POLL 10/23/88 */ {
       ...
       LOGICAL speed,racespeed; /* whether Speed-Entry was used */
       char date[12],time[8];
       char fname[13];
       FILE *fopen();
       ...
       if (iunit < 0 || indxpr < 0) {
             fprintf(stderr,"*** Skipping SPOL. Run canceled.\n");
             goto done;
             };
       ...
       speed = FALSE;        /* 10-23-88 */
       racespeed = FALSE;
       for (j=1;j<(ncand+1);j++) {
             fscanf(flprecvote,"%ld ",&vote[j]);
             if (vtpoll <= 0 && vote[j] > 0) speed = TRUE;        /* SPEED */
             };
       fprintf(stderr,"Precinct %d read from file %12.12s.\n",indxpr,fname);
       if (iprnum != vtprec | iunit != vtunit) goto x801;
       if (ifound == 3) {
             fprintf(stderr,"This precinct was speed-entered.\n");
             if (speed != TRUE)
                  fprintf(stderr,"*** Speed entry was all zero.\n");
             };
...
x530:   totoff = 0L;
             ...
             /* check if there was speed entry for this race */
             if (! speed) goto candq;     /* 10-23-88 */
             if (racespeed) goto candq;   /* prevent second iteration if bad Speed-Entry of race */
             racespeed = FALSE;
             for (k=istart;k<(iend+1);k++) {
                  if (! racespeed && vote[k] > 0L)
                      racespeed = TRUE;
                  if (vote[k] > vtpoll) {
                      fprintf(stderr,"*** Bad speed entry for this race.\n");
                      fprintf(stderr,"*** A candidate has more that total vote at poll.\n");
             ...
             goto x580;  /* when using Speed-Entry, skip to verify */
