...
#include "unitinfo.h"
main()
/* edit precinct results for corrections during count night entry */
/* version of VEDT program without writein capability */
/* created 10-23-88 */ {
...
x526:   gtoffi(j,report,0);
             ...
             fprintf(stderr,"%s\n\n",report);
             blkbuf(80,report);
             askchange("this office",&change,&quit,TRUE,FALSE);
...
x536:   fprintf(stderr,
             "\nChange the current results (Y,N,DONE) ?\n");
...
x580:   sclear();
       ...
       "\n                                         more to come...\n");
                      if (! confrm()) {
                         sclear();
                         fprintf(stderr,
                      "*** Re-edit results for this office:\n");
                         goto x526;
                         };
                      sclear();
                      gtoffi(j,report,0);
