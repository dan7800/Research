/** lans - EJCN88 version to print selected races in late corrections **/
#include <stdio.h>
#include "weltab.h"
#include "filedecl.h"
#include "candnm.h"
...
main()
/* produce the official canvass report */
/* first state report (lansing book) then local report (unlansing book) */ {
       int cstart[4],clen[4],cbreak;
       long int prtotl[MAXPRECINCT];
       int lincr,lcand,rincr,rcand;
       int i,j,k,m,ierr,xprec,namlen;
       ...
       ipage = 0;
       welcom(date,time);
       fprintf(stderr,"Preparing State Canvass Report...\n\n");
       ...
       smove(headc,12,"Official Canvass Report",0,23);
       smove(headc,38,electn,0,50);
       smove(headc,93,date,0,12);
       smove(headc,106,time,0,8);
       smove(headc,117,"Page",0,4);
       smove(fprecname,0,"PRECxxxx.VOT",0,12);
       fprecname[12] = '\0';
       smove(fofficename,0,"RACExxxx.SBK",0,12);
       fofficename[12] = '\0';
       /* first pass - state report (lansing book) */
       dolans = TRUE;
       /* main loop - get next office */
x100:  noop();
       for (j=1;j<(noffic+1);j++) {
/** EJCN88 addition **/
             if (j != 1 && j != 14 && j != 15 && j != 72 &&
                 j != 30 &&
                 j != 73 && j != 77 && j != 121 && j != 130
                  ) continue;
/** EJCN88 addition **/
             getoff(j);
             /* check which book office is meant for */
             if (dolans && ! prtinf[STATE]) continue;
             if (! dolans && prtinf[STATE]) continue;
             gtoffi(j,offnam,0);
...
x105:   outpos = 23;
             ...
             if (offinf[WRITEINCOUNT] > 0)
                  icand += offinf[WRITEINCOUNT];
             if (icand <= 0) failure(20);
             ...
             for (k=1;k<(icand+1);k++) {
                  ...
                  if ((namlen == 3 && lscomp(cname,0,"Yes",0,3))
                   || (namlen == 2 && lscomp(cname,0,"No",0,2))) {
                      ilen = max(namlen,12);
                      if (outpos+ilen >= 130)
                         prtpag(j,&ipage,date,time,prtotl);
...
x215:                  len2 = namlen - cstart[cbreak];
       ...
       /* go back for second pass - unlansing book */
       if (dolans) {
             fprintf(stderr,"\n\nPreparing Local Canvass Report...\n\n");
             smove(fofficename,8,".LBK",0,4);
             dolans = FALSE;
             goto x100;
             };
x900:  fcloseall(); }
/* ===================== */
/** prtpag **/
void prtpag(ioffic,fpage,date,time,prtotl)
...
x118:   if (lastun == vtunit) goto x150;
             ...
             if (lines+2 >= LANDSCAPE)
                  lhead(ioffic,&lines,&ipage,date,time);
             lprint(filereport);
             lines += 2;
...
x152:   if (iward > 0) {
             ...
             fprintf(stderr,"       %22.22s\r",longrep);
x112:   if (nform <= 0) failure(8);
             for (k=1;k<(nform+1);k++) {
                  if (form[CAND][k] <= 0) goto x117;
                  form[NVOTE][k] = vote[form[CAND][k]];
...
char chtime[]; {
       ...
       return; }
/** lhead **/
void lhead(ioffic,lines,ipage,date,time)
/* print header for state and local canvass reports */
int    ioffic;
int    *lines;
int    *ipage;
char   date[],time[]; {
