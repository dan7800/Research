...
public class DoMemberCardReplace extends ActionDo implements Action
{
			...
			throws java.io.IOException, javax.servlet.ServletException
	{
		...
		if (formName == null)
		{
			return new ActionRouter("loyal-membercard-replace-page");
		}
	...
	public void fnSetCardNo(HttpServlet servlet, HttpServletRequest req, HttpServletResponse res) throws Exception
	{
		...
		ReplaceMemberCardSession cmcs = (ReplaceMemberCardSession) session.getAttribute("loyal-membercard-replace-session");
		...
		if (cardNo != null)
		{
			cmcs.setMemberCard(cardNo);
		      ...
		      if(option.equals("setCard"))
		      {
		         ...
		         try
		         {
		            Long lCardPkid = new Long(cardPkid);
		            MemberCardObject cardObj = MemberCardNut.getObject(lCardPkid);
		            if(cardObj!=null)
		            {
		            ...
		            {
		               throw new Exception("Invalid Member Card!");
		            }
	...
	{			
		...
		if(invoiceNum != null && "replace".equals(invoiceStatus))
		{
			Long invNum = new Long(invoiceNum);					 
			String invoiceItems = "";
			String invoiceItemsArray[] = AppConfigManager.getProperty("LOYAL-CARD-REPLACE-CHECK-INVOICE").split(",");
		...
		if (memberCard != null)
		{
			...
			pointsDelta.tx_type = CustMembershipPointsDeltaBean.TX_REPLACE;
			...
			pointsDelta.points_type = CustMembershipPointsDeltaBean.TX_REPLACE;
			...
			pointsDelta.description = "Replace";
			...
			MemberCardAuditTrailObject cardTrail = new MemberCardAuditTrailObject();
	      	...
	      	cardTrail.auditType = MemberCardAuditTrailBean.AUDIT_TYPE_REPLACE;
		...
		atObj.remarks = "Replace Member Card " + cardNo + "";
	...
	public void fnCreateSessionObject(HttpServlet servlet, HttpServletRequest req, HttpServletResponse res)
	{
		...
		{
			Integer userId = (Integer) session.getAttribute("userId");
			cmcs = new ReplaceMemberCardSession(userId);
			session.setAttribute("loyal-membercard-replace-session", cmcs);
		}
		String cardNo = req.getParameter("cardNo");
		if (cardNo == null)
		{
