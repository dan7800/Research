...
public class DoProcurementPkgItemAdd implements Action
{
	String strClassName = "DoProcurementPkgItemAdd";
			...
			throws java.io.IOException, javax.servlet.ServletException
	{
		...
		return new ActionRouter("supp-procurement-add-pkg-items-page");
	}
	...
	protected void fnAddPackage(HttpServlet servlet, HttpServletRequest req, HttpServletResponse res)
	{
		...
		try
		{
			 ...
			 * effDate_day);
			 */
			Log.printVerbose("Adding new Purchase Package Item");
			 ...
			 * (PurchaseItemNut.isPOSSvcExist(pkg_code, new
			 ...
			 */
			/*
			 * String lItemType = PurchaseItemBean.TYPE_PKG; String lCurrency =
			 * "MYR"; // default
			 */
			...
			// continue to create the PurchaseItem Object for the package code
			/*
			 * if (newPackage != null) { // Get the packageItemId Integer
			 * lItemPKId = newPackage.getPkid();
			 * 
			 * PurchaseItemHome lPurchaseItemHome = PurchaseItemNut.getHome();
			 * 
			 * try { PurchaseItem newPurchaseItem =
			 * lPurchaseItemHome.create(lItemPKId, lItemType, lCurrency, new
			 * BigDecimal(unit_px_std), new BigDecimal(unit_px_discounted), new
			 * BigDecimal(unit_px_min), new Timestamp(effDateInMillis),
			 * tsCreate, usrid); // if it reaches here, it has successfully
			 * created the PurchaseItem if(newPurchaseItem != null) { String
			 * rtnMsg = "Successfully created PurchaseItem for Package Code = " +
			 * pkg_code; Log.printDebug(rtnMsg); req.setAttribute("rtnMsg",
			 * rtnMsg); } } catch (Exception ex) { // rollback the newly created
			 * Package Item newPackage.remove();
			 *  // rethrow throw ex; }
