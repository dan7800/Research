...
public class DoProcurementPkgItemEdit implements Action
{
	String strClassName = "DoProcurementPkgItemEdit";
			...
			throws java.io.IOException, javax.servlet.ServletException
	{
		...
		fnGetParams(servlet, req, res);
		return new ActionRouter("supp-procurement-edit-pkg-items-page");
	}
	...
	protected void fnGetParams(HttpServlet servlet, HttpServletRequest req, HttpServletResponse res)
	{
		 ...
		 * packageCode [2] (PurchaseItem) purchasePkgItem corresponding to the
		 * editPkg [ if exist ] [3] (Vector) vecItemCodes, vecItemNames,
		 * vecInvQuantities [4] (Vector) vecSvcCodes, vecSvcNames,
		 * vecSvcQuantities
		 * 
		 */
		...
		{ // super huge try block
			...
			if (lPackage != null)
			{
				...
				// Now try to obtain the purchasePkgItem corresponding to
				// editPkg
				PurchaseItem purchasePkgItem = PurchaseItemNut.getPurchaseItem(lPackage.getPkid(),
						PurchaseItemBean.TYPE_PKG);
				if (purchasePkgItem != null)
				{
					req.setAttribute("purchasePkgItem", purchasePkgItem);
				}
				...
				// how to get a unique purchasePkgItem???
	...
	protected void fnUpdatePackage(HttpServlet servlet, HttpServletRequest req, HttpServletResponse res)
	{
		...
		// String purchase_uom = req.getParameter("pos_uom");
		...
		try
		{
			...
			PurchaseItem lPurchaseItem = PurchaseItemNut.getPurchaseItem(new Integer(packageId),
					PurchaseItemBean.TYPE_PKG);
			...
			} else
			{
				...
				// if it reaches here, it has successfully created the
				// PurchaseItem
				if (newPurchaseItem != null)
				{
					String rtnMsg = "Successfully created PurchaseItem for Package Code = " + packageCode;
					Log.printDebug(rtnMsg);
					req.setAttribute("rtnMsg", rtnMsg);
				}
		...
		{
			String rtnMsg = "Update PurchaseItem for Package Code = " + packageCode + " -  Failed: " + ex.getMessage();
			Log.printDebug(rtnMsg);
			req.setAttribute("rtnMsg", rtnMsg);
		} // end try-catch
	} // end fnUpdatePackage
