...
public class ServiceUtil {
    ...
    // Begin neogia specific : helper method to concat result map
    /**
     * Add success message, success message list, error message, error message list,
     * and error message map of nestedResult to result.
     * 
     * @param result
     * @param nestedResult
     * @return result
     */
    public static Map addToResult(Map result, Map nestedResult) {
        List successList = (List) result.get(ModelService.SUCCESS_MESSAGE_LIST);
        if (successList == null) {
            successList = FastList.newInstance();
        }
        
        List errorList = (List) result.get(ModelService.ERROR_MESSAGE_LIST);
        if (errorList == null) {
            errorList = FastList.newInstance();
        }

        Map errorMap = (Map) result.get(ModelService.ERROR_MESSAGE_MAP);
        if (errorMap == null) {
            errorMap = FastMap.newInstance();
        }
        
        if (nestedResult != null) {
            if (nestedResult.get(ModelService.SUCCESS_MESSAGE) != null) {
                successList.add(nestedResult.get(ModelService.SUCCESS_MESSAGE));
            }
            if (nestedResult.get(ModelService.SUCCESS_MESSAGE_LIST) != null) {
                successList.addAll((List) nestedResult.get(ModelService.SUCCESS_MESSAGE_LIST));
            }
        ...
        if (!successList.isEmpty()) {
            result.put(ModelService.SUCCESS_MESSAGE_LIST, successList);
        }
    ...
    // End neogia specific : helper method to concat result map
