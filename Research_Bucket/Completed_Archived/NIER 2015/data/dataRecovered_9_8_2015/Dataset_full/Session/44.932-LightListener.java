...
package vu.globe.rts.comm.lwconn;
...
public class LightListener extends cfgListenerSkel
{
  /**
  ...
  public void configure (String config_data) throws configureErrors
  {
    ...
    try {
      ...
      _mux_impl = cfg.get (LightConfig.MUX_IMPLEMENTATION);
      _mux_init = cfg.get (LightConfig.MUX_INITIALISATION);
      _mux_stack = cfg.get (LightConfig.PROT_STACK);
      _mux_prot_id = new ProtStack (_mux_stack).remove();
    }
  ...
  public void exportContact (String contact) throws commErrors
  {
    ...
    if (contact != null) {
      LightAddress la = new LightAddress (contact, _mux_prot_id);
    ...
    try {
      ...
      if (contact == null) { // pick a port
        ...
        _contact_point = new LightAddress (base_address, _port, _mux_prot_id).
				toString();
      }
