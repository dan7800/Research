...
public class DoFloristEditPOFromSO extends ActionDo implements Action
{
	...
	{		
		...
		try
		{
			...
			EditPOForm epof = (EditPOForm) session.getAttribute("distribution-edit-po-from-so-form");		
			if (epof == null)
			{			
				epof = new EditPOForm(poId, userid);		
				session.setAttribute("distribution-edit-po-from-so-form", epof);
			}
			...
			epof.setIntefloraDetails(soObj.interfloraReference, soObj.interfloraRemarks, 
										soObj.interfloraExeMem, soObj.interfloraSendingNo, soObj.interfloraOtherInstructions, "");												
	...
	protected void setIntefloraDetails(HttpServlet servlet, HttpServletRequest req, HttpServletResponse res)
	{
		...
		HttpSession session = req.getSession();
		EditPOForm epof = (EditPOForm) session.getAttribute("distribution-edit-po-from-so-form");
		epof.setIntefloraDetails(interfloraNo, interfloraRemarks, interfloraExeMem, interfloraSendingNo, interfloraOtherInstructions, "");
	}
	...
	protected void fnSetDocRowPrice3(HttpServlet servlet, HttpServletRequest req, HttpServletResponse res)
	{
		...
		epof.setDocRowPrice3(docRowKey, itemPrice3);
