...
public class DoSalesOrderFast extends ActionDo implements Action
{
...
//		EditSalesOrderSession esos = (EditSalesOrderSession) session.getAttribute("dist-sales-order-fast-form");
			...
			throws Exception
	{
        ...
        {        	              
			...
			esos.setOrderType2(SalesOrderIndexBean.SO_TYPE2_FLORIST);
			esos.setReceiptMode("");
			esos.setPaymentRemarks("");
			esos.setPaymentStatus("");			
...
//            Vector vecUnsavedOrder = new Vector(DocumentProcessingItemNut.getObjects(queryUnsavedOrder));
								...
								HttpServletResponse res) throws Exception
   {
	...
	private void fnRepeatOrder(HttpServlet servlet, HttpServletRequest req, HttpServletResponse res) throws Exception
	{
		...
		csos.setDocRows(soObj.getDocRowsForRepeatOrders());
	...
	private String fnSetAllDetails(HttpServlet servlet, HttpServletRequest req, HttpServletResponse res)
	{
						...
						{						
							try
							{
								docTrail = DocumentProcessingItemNut.appendDocTrail(
										"ITEM-REMARKS: "+theDocRow.itemCode, theDocRow.getRemarks(), docrowRemarks[cnt1], docTrail);
							...
							try
							{
								if(theDocRow.getDiscount().compareTo(new BigDecimal(docrowDiscount[cnt1])) != 0 )
								{
									docTrail = DocumentProcessingItemNut.appendDocTrail(
											"ITEM-DISCOUNT: "+theDocRow.itemCode, theDocRow.getDiscount().toString(), docrowDiscount[cnt1], docTrail);									
								}
							...
							try
							{
								if(theDocRow.getQty().compareTo(new BigDecimal(docrowQty[cnt1])) != 0 )
								{
									docTrail = DocumentProcessingItemNut.appendDocTrail(
											"ITEM-QTY: "+theDocRow.itemCode, theDocRow.getQty().toString(), docrowQty[cnt1], docTrail);
								}								
			...
			String receiverPhone1 = req.getParameter("Rtelephone1_Prefix") + "-" + req.getParameter("Rtelephone1") 
									+ "Ext" + req.getParameter("Rext");					
			...
			String receiverStateList = req.getParameter("receiverStateList");
			String receiverStateBox = req.getParameter("receiverStateBox");

			String receiverState = "";
			if(receiverStateList != null && !"".equals(receiverStateList))
				receiverState = receiverStateList;
			else
				receiverState = receiverStateBox;
			...
			while (itemCode != null && itemCode.length() > 2)								
			{
				...
				System.out.println("Inside updateAllDetails -> item code : "+itemCode);
