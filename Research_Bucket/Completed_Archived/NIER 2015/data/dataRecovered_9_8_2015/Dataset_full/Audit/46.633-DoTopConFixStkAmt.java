...
public class DoTopConFixStkAmt implements Action
{
			...
			throws java.io.IOException, javax.servlet.ServletException
	{
		...
		try
		{
			...
			if (stockRptPath != null && stockRptPath.trim().equals(""))
			...
			String queryStkBal = "select bal,unit_cost_ma from inv_stock where itemid = "
					+ "(select pkid from inv_item where item_code = ?) and locationid=1000";
			PreparedStatement stkBalStmt = empCon.prepareStatement(queryStkBal);
			String queryStkNonZeroBal = "select i.item_code,s.bal,s.unit_cost_ma from inv_stock s inner join inv_item i on (i.pkid=s.itemid)"
					+ " where s.locationid=1000 and s.bal != 0 ";
			...
			for (Enumeration e = stkRptReader.stockTable.elements(); e.hasMoreElements();)
			{
				...
				if (rsStkBal.next())
				{
					...
					{
						bdUCostDiff = bdUCostMA.subtract(thisStk.totalCost.divide(actualBal, 4,
...
//							StockNut.adjustMA(usrid, thisItem.getPkid(), iDefLocId, iDefPCCenterId, actualBal,
//									thisStk.totalCost.divide(actualBal, 4, BigDecimal.ROUND_HALF_UP), strDefCurr,
//									"To fix discrepancies in Opening Stock Amount", TimeFormat.getTimestamp(), usrid);
						} // end if (thisItem == null)
					} // end if (bdBalDiff.signum() != 0 ||
						// bdCostDiff.signum() != 0 )
					else
					{
						// System.out.println(thisStk.custCode + ": OK");
					}
				} else
				{
			...
			// Backward search, i.e given EMP, search AR
			countBalDiff = 0;
			countCostDiff = 0;
			...
			while (rsStkNonZeroBal.next())
			{
				...
				if (thisBal.signum() > 0)
				{
					...
					{
/*						StockNut.out(usrid, thisItem.getPkid(), iDefLocId, iDefPCCenterId, thisBal, new BigDecimal(0),
								strDefCurr, "", new Long(0), "To fix discrepancies in Opening Stock Amount", TimeFormat
										.getTimestamp(), usrid, StockDeltaBean.NS_INVENTORY,
								StockDeltaBean.TT_ADJUSTMENT, new Vector());
*/					}
				} else
				{
					Log.printVerbose("thisBal > 0 ... ");
					...
					{
/*						StockNut.in(usrid, thisItem.getPkid(), iDefLocId, iDefPCCenterId, thisBal.abs(),
								thisUnitPriceMA, strDefCurr, "", new Long(0),
								"To fix discrepancies in Opening Stock Amount", TimeFormat.getTimestamp(), usrid,
								StockDeltaBean.NS_INVENTORY, StockDeltaBean.TT_ADJUSTMENT, new Vector());
*/					}
				}
			} // end while
		} // end huge-try
