...
public class MembershipCampaignEditForm extends java.lang.Object implements Serializable
{
	...
	public boolean dirtyFlag = false; /// when there's modifications to the campaign, dirty flag will be set to true
	...
	public void loadCampaign(Integer campaignId) 
			throws Exception
	{
		this.treeWidgetRules.clear();
		if(this.dirtyFlag && this.campaign!=null){ throw new Exception("You have not saved the previous campaign!");}
		...
		for(int cnt1=0;cnt1<this.campaign.vecRules.size();cnt1++)
		{
			...
			/// need to start creating the widget
	...
	public void setDetails(String code, String name, String description, Timestamp tsStart, Timestamp tsEnd,
									String membershipLogic, String branchLogic, String branchConditions, String status)
	{
	...
	public void setSequence(String guid, Integer iSequence)
	{
		...
		for(int cnt1=0;cnt1<vecWidgetRules.size();cnt1++)
		{
			MembershipCampaignWidgetRewardRulesForm widgetRulesForm = 
								(MembershipCampaignWidgetRewardRulesForm) vecWidgetRules.get(cnt1);
	...
	public synchronized void saveCampaign() throws Exception
	{
		...
		try
		{
			...
			/// first of all, update/create the existing WidgetRules
			Vector vecWidgetRules = new Vector(this.treeWidgetRules.values());
			for(int cnt1=0;cnt1<vecWidgetRules.size();cnt1++)
			{
			...
			/// next, delete from DB the rules that have been removed.
			for(int cnt1=0;cnt1<this.campaign.vecRules.size();cnt1++)
			{
