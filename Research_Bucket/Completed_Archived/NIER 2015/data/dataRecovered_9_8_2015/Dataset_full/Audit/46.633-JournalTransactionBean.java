...
public class JournalTransactionBean implements EntityBean
{
	...
	// public static final Integer TYPEID_DEFAULT = new Integer("0");
	public static final String STATE_CREATED = "created";
	private JournalTransactionObject valObj = null;

	/*
	 * private Long pkId; private Long prevJournalTxnId; private Long
	 * nextJournalTxnId; private String txnCode; private Integer pcCenterId;
	 * private Integer batchId; private Integer typeId; private String status;
	 * private String name; private String description; private BigDecimal
	 * amount; private Timestamp transactionDate; private String docRef; private
	 * Long docKey; private String state; private Integer userIdCreate; private
	 * Integer userIdEdit; private Integer userIdCancel; private Timestamp
	 * timestampCreate; private Timestamp timestampEdit; private Timestamp
	 * timestampCancel;
	 */
	public Long getPkId()
	{
	...
	public Long ejbCreate(String txnCode, Integer pcCenterId, Integer batchId, Integer typeId, String name,
			String description, BigDecimal amount, Timestamp transactionDate, String docRef, Long docKey,
			Integer userId, Timestamp tsTime) throws CreateException
	{
		Long newPkId = null;
		Log.printVerbose(strObjectName + " In ejbCreate");
		newPkId = insertNewRow(txnCode, pcCenterId, batchId, typeId, name, description, amount, transactionDate,
				docRef, docKey, userId, tsTime);
		if (newPkId != null)
		{
	...
	public void ejbPostCreate(String txnCode, Integer pcCenterId, Integer batchId, Integer typeId, String name,
			String description, BigDecimal amount, Timestamp transactionDate, String docRef, Long docKey,
			Integer userId, Timestamp tsTime)
	{
	}
	...
	public Collection ejbFindEJBsGiven(String field1, String value1, String field2, String value2, String field3,
			String value3, String field4, String value4, String field5, String like5, String field6,
			Timestamp timeAfterEq, Timestamp timeBefore) throws FinderException
	{
		Collection col = selectEJBsGiven(field1, value1, field2, value2, field3, value3, field4, value4, field5, like5,
				field6, timeAfterEq, timeBefore);
		return col;
	}
	...
	public Vector ejbHomeGetValueObjectsGivenDate(String fieldName1, String value1, String fieldName2, String value2,
			String fieldName3, String value3, Timestamp ts1, Timestamp ts2)
	{
		Log.printVerbose(strObjectName + " In ejbHomeGetValueObjectsGivenDate");
		Vector vecValObj = selectValueObjectsGivenDate(fieldName1, value1, fieldName2, value2, fieldName3, value3, ts1,
				ts2);
		Log.printVerbose(strObjectName + " Leaving ejbHomeGetValueObjectsGivenDate");
		return vecValObj;
	}

	public Vector ejbHomeGetImbalanceTxn(String field1, String value1, String field2, String value2, String field3,
			String value3)
	{
		Vector vecJTxn = null;
		try
	...
	private Long insertNewRow(String txnCode, Integer pcCenterId, Integer batchId, Integer typeId, String name,
			String description, BigDecimal amount, Timestamp transactionDate, String docRef, Long docKey,
			Integer userId, Timestamp tsTime)
	{
		...
		try
		{
			...
			String sqlStatement = " INSERT INTO " + TABLENAME
					+ " (pkid, prev_journaltxnid, next_journaltxnid, txncode, pccenterid, "
					+ " batchid, typeid, status, name, description, amount,"
					+ " transactiondate, doc_ref, doc_key, state, userid_create, userid_edit, "
					+ " userid_cancel, createtime, edittime, canceltime)"
					+ " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	private void storeObject()
	{
		...
		try
		{
			String sqlStatement = "UPDATE " + TABLENAME + " SET prev_journaltxnid = ?, next_journaltxnid = ?,"
					+ " txncode = ?, pccenterid = ?, batchid = ?, typeid = ?,"
					+ " status = ?, name = ?, description = ?, amount = ?, transactiondate = ?, "
					+ " doc_ref = ?, doc_key = ?, state = ?, userid_create = ?,"
					+ " userid_edit = ?, userid_cancel = ?, createtime = ?, edittime = ?, canceltime = ? "
					+ " WHERE pkid = ?";
			cn = ds.getConnection();
			ps = cn.prepareStatement(sqlStatement);
	...
	private Collection selectObjectsGiven(String fieldName, String value)
	{
		...
		try
		{
			...
			String sqlStatement = "SELECT pkid FROM " + TABLENAME + " WHERE " + fieldName
					+ " = ? ORDER BY transactiondate, pccenterid, pkid ";
			cn = ds.getConnection();
			ps = cn.prepareStatement(sqlStatement);
	...
	private Collection selectEJBsGiven(String field1, String value1, String field2, String value2, String field3,
			String value3, String field4, String value4, String field5, String like5, String field6,
			Timestamp timeAfterEq, Timestamp timeBefore)
	{
		DataSource ds = getDataSource();
		Connection cn = null;
		PreparedStatement ps = null;
	...
	private Vector selectImbalanceTxn(String field1, String value1, String field2, String value2, String field3,
			String value3)
	{
		DataSource ds = getDataSource();
		Connection cn = null;
		...
		{
			/*
			 * String sqlStatement = "SELECT pkid, prev_journaltxnid,
			 * next_journaltxnid, txncode, pccenterid, batchid, typeid, status,
			 * name, description, amount," + " transactiondate, userid_create,
			 * userid_edit, userid_cancel, createtime, edittime, canceltime FROM " +
			 * TABLENAME + " WHERE " + fieldName1 + " = ?";
			 */
			String sqlStatement = "	SELECT * FROM " + TABLENAME
					+ " WHERE pkid in (SELECT tmp1.pkid FROM (SELECT jt.pkid," + "  sum(je.amount) AS je_amount "
					+ " FROM acc_journal_transaction AS jt INNER JOIN acc_journal_entry "
					+ " AS je ON (jt.pkid = je.journaltxnid) GROUP BY jt.pkid) "
					+ " AS tmp1 INNER JOIN acc_journal_transaction "
					+ " AS jtxn2 ON (tmp1.pkid = jtxn2.pkid) WHERE je_amount>'0' OR je_amount<'0' " + " AND " + field1
					+ "='" + value1 + "' ";
			if (field2 != null && value2 != null)
			{
	...
	private Vector selectValueObjectsGiven(String fieldName1, String value1, String fieldName2, String value2)
	{
		...
		try
		{
			String sqlStatement = "SELECT pkid, prev_journaltxnid, next_journaltxnid, txncode, pccenterid, "
					+ " batchid, typeid, status, name, description, amount,"
					+ " transactiondate, doc_ref, doc_key, state, userid_create, userid_edit, "
					+ " userid_cancel, createtime, edittime, canceltime FROM " + TABLENAME + " WHERE " + fieldName1
					+ " = ?";
			...
			String fieldName3, String value3, Timestamp ts1, Timestamp ts2)
	{
		...
		try
		{
			String sqlStatement = "SELECT pkid, prev_journaltxnid, next_journaltxnid, txncode, "
					+ " pccenterid, batchid, typeid, status, name, description, amount,"
					+ " transactiondate, doc_ref, doc_key, state, userid_create, userid_edit, "
					+ " userid_cancel, createtime, edittime, canceltime FROM " + TABLENAME + " WHERE " + fieldName1
					+ " = ?";
