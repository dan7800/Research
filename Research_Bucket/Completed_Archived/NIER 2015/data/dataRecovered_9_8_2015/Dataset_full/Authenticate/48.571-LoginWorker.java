...
public class LoginWorker {
    ...
    public static void doBasicLogin(GenericValue userLogin, HttpServletRequest request) {
        ...
        // Begin neogia specific : task FR#1363772 set user's locale and webPreferences at login       
        String locale = request.getParameter("locale");
        if (locale != null && locale.length() > 0) {            
            CommonEvents.setSessionLocale(request, null);
        }
        
        //TODO: web user preferences must be moved to framework
        //WebUserPreferencesHelper.loadWebUserPreferencesInSession(request);
        loadWebUserPreferencesInSession(request);
        // End neogia specific : task FR#1363772 set user's locale and webPreferences at login
    ...
    // Begin Neogia specific 
    public static void loadWebUserPreferencesInSession(HttpServletRequest request) {
        GenericDelegator delegator = (GenericDelegator) request.getAttribute("delegator");
        HttpSession session = request.getSession();
        GenericValue userLogin = (GenericValue) session.getAttribute("userLogin");

        List webUserPreferenceList = null;
        try {
            webUserPreferenceList = delegator.findByAnd("WebUserPreference", UtilMisc.toMap("userLoginId", userLogin.get("userLoginId"), "partyId", userLogin.get("partyId")));
        } catch (GenericEntityException ex) {
            String errMsg = "Failed to get web user preferences from database : " + ex.getMessage();
            Debug.logError(errMsg, module);
        }
        
        Map wupMap = new HashMap();
        if (webUserPreferenceList != null) {
            for (Iterator it = webUserPreferenceList.iterator(); it.hasNext();) {
                GenericValue wup = (GenericValue) it.next();
                wupMap.put(wup.getString("webPreferenceTypeId"), wup.getString("webPreferenceValue"));
            }
        }
        if (wupMap.get("SCREEN_THEME") != null &&  ! "WUP_THM_NEO".equals(wupMap.get("SCREEN_THEME") ) )
            session.setAttribute("mainDecoratorLocation","component://images/webapp/images/themes/"+(String)wupMap.get("SCREEN_THEME")+"/CommonScreens.xml");
        else
            session.removeAttribute("mainDecoratorLocation");
        
        String viewSizeSt = (String) wupMap.get("VIEWSIZE_LIST");
        if ( viewSizeSt != null && viewSizeSt.length()>0)
            wupMap.put("VIEWSIZE_LIST", viewSizeSt.substring(8) );

        viewSizeSt = (String) wupMap.get("VIEWSIZE_SUBLIST");
        if ( viewSizeSt != null && viewSizeSt.length()>0)
            wupMap.put("VIEWSIZE_SUBLIST", viewSizeSt.substring(8) );

        
        session.setAttribute("webUserPreferences", wupMap);
    }
    // End Neogia specific
}
