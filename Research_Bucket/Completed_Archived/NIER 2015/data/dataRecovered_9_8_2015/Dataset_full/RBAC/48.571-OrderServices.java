...
import org.ofbiz.common.note.developed.NoteData;
import org.ofbiz.common.note.developed.NoteDataServices;
import org.ofbiz.facility.stockevent.developed.OrderStockEventPlanned;
import org.ofbiz.facility.stockevent.developed.OrderStockEventPlannedQuery;
import org.ofbiz.facility.stockevent.developed.OrderStockEventPlannedServices;
import org.ofbiz.order.order.developed.OrderHeader;
import org.ofbiz.order.order.developed.OrderHeaderNote;
import org.ofbiz.order.order.developed.OrderHeaderNoteServices;
import org.ofbiz.order.order.developed.OrderHeaderServices;
import org.ofbiz.order.order.developed.OrderItem;
import org.ofbiz.order.order.developed.OrderItemShipGroup;
import org.ofbiz.order.order.developed.OrderItemShipGroupAssoc;
import org.ofbiz.order.order.developed.OrderItemShipGroupAssocServices;
import org.ofbiz.order.order.developed.OrderItemShipGroupAssocViewQuery;
import org.ofbiz.order.order.developed.OrderItemShipGroupAssocWorker;
import org.ofbiz.order.order.developed.OrderItemShipGroupServices;
import org.ofbiz.order.order.developed.OrderItemStatus;
import org.ofbiz.order.order.developed.OrderStatus;
import org.ofbiz.order.order.developed.OrderStatusServices;
import org.ofbiz.order.order.developed.OrderType;
import org.ofbiz.order.shoppingcart.ShoppingCartHelper;
import org.ofbiz.order.shoppingcart.ShoppingCart.CartShipInfo;
import org.ofbiz.party.party.developed.Party;
import org.ofbiz.party.party.developed.UserLogin;
...
public class OrderServices {
    ...
    private static final int decimals = UtilNumber.getBigDecimalScale("stock.quantity.decimals");
    private static final int rounding = UtilNumber.getBigDecimalRoundingMode("stock.quantity.rounding");
    // End Neogia specific
    ...
    public static Map createOrder(DispatchContext ctx, Map context) {
        ...
        while (normalizedIter.hasNext()) {
            ...
            if ("SALES_ORDER".equals(orderTypeId)) {
                ...
                //Begin Neogia Specific : this check is done during orderStockEvent creation or update 
                /*
                try {
                    // ofbiz version
                    ...
                    // neogia version
                    Map invReqResult = dispatcher.runSync("isStoreInventoryAvailableOrNotRequired", UtilMisc.toMap("productStoreId", productStoreId, "productId", product.get("productId"), "product", product, "quantity", currentQuantity,"releaseDate",estimatedDeliveryDate));
                */
                Map invReqResult = ServiceUtil.returnSuccess();
                invReqResult.put("availableOrNotRequired","Y");
                ...
                */
                //End Neogia Specific : this check is done during orderStockEvent creation or update 
        ...
        // begin Jira ofbiz-382
        //Customer process to generate the orderId
        try {
            // neogia modification added salesChannelEnumId as parameters
            Map res = dispatcher.runSync("getHookOrderId", UtilMisc.toMap("orderTypeId", orderTypeId, "productStoreId", productStoreId, "salesChannelEnumId", context.get("salesChannelEnumId")));
        ...
        // Begin neogia specific : use the standard ofbiz process only if Hook return a null value (Jira ofbiz-382)
        /*
        if (UtilValidate.isNotEmpty(orgPartyId)) {
        */
        if (UtilValidate.isEmpty(orderId) && UtilValidate.isNotEmpty(orgPartyId)) {
        // End neogia specific : use the standard ofbiz process only if Hook return a null value (Jira ofbiz-382)
            Map getNextOrderIdContext = UtilMisc.toMap("partyId", orgPartyId, "userLogin", userLogin);

            // neogia modification use the getNextOrderId service only for SaleOrder
            /*
            if ((orderTypeId.equals("SALES_ORDER")) || (productStoreId != null)) {
            */
            if ((orderTypeId.equals("SALES_ORDER")) && (productStoreId != null)) {
                getNextOrderIdContext.put("productStoreId", productStoreId);
                // Neogia modification : included this line in the < < orderTypeId.equals("SALES_ORDER")> > test because not usable for purchase
            /*
            }
            ...
            */
            try {
                ...
                // Neogia modification : end of  included this line in the < < orderTypeId.equals("SALES_ORDER")> > test because not usable for purchase
            }
        }
        ...
        if (UtilValidate.isEmpty(orderId)) {
            ...
            // neogia modification add the orderType in the nextSeqName, for purchase
            /*
            orderId = delegator.getNextSeqId("OrderHeader");
            */
            orderId = delegator.getNextSeqId(orderTypeId + ".OrderHeader");
        }
...
// FR 1444861 no tax         
        if (context.get("noTax") != null){
            orderHeader.set("noTax", context.get("noTax"));
        }
// FR 1444861     
        
        // Begin Neogia Specific : FR1517372 - orderReceivedDate management
        Timestamp orderReceivedDate = (Timestamp) context.get("orderReceivedDate");
        if (orderReceivedDate != null)
        	orderHeader.set("orderReceivedDate", orderReceivedDate);
        // End Neogia Specific : FR1517372 - orderReceivedDate management
        ...
        while (oi.hasNext()) {
            ...
            // Begin Specific Neogia FR#1427908 : Add revision Number Management step 2
            Timestamp now = UtilDateTime.nowTimestamp();
            String productId = orderItem.getString("productId");
            GenericValue productRevision = OrderReadHelper.getProductRevision(delegator, productId, now);
            if(productRevision != null){
                String revisionId = productRevision.getString("revisionId");
                orderItem.set("revisionId",revisionId);
            }
            // End Specific Neogia FR#1427908 : Add revision Number Management step 2
        ...
        if (workEfforts != null && workEfforts.size() > 0) {
            ...
            while (we.hasNext()) {
                ...
                if(techDataCalendar == null ) {
                    ...
                    // Begin neogia specific : Use Neogia TechDataCalendar entities not ofbiz one, some there is some minor changes (entity and field name)
                    /*
                    techDataCalendar.set("calendarId", calendarId);
                    */
                    techDataCalendar.set("idName", calendarId);
                    // End neogia specific : Use Neogia TechDataCalendar entities not ofbiz one, some there is some minor changes (entity and field name)
...
//
                ...
                //  Begin Neogia specific: FR#1706904 - included code in the while has been removed to a separate method
                /*
                while (--dayCount >= 0)    {
                */
                String removedCode = null;
                while (--dayCount >= 0 && removedCode != null )    {
                // End Neogia specific: FR#1706904 - included code in the while has been removed to a separate method
                    ...
                    try {
                        ...
                        */    
                    	techDataCalendarExcDay = EntityUtil.getFirst(delegator.findByAnd("CalendarException",
                    			UtilMisc.toMap("tdcalIdName", fixedAsset.get("calendarId"), "exceptionDate", exceptionDateStartTime)) );
                        // End neogia specific : Use Neogia TechDataCalendar entities not ofbiz one, some there is some minor changes (entity and field name)
                    }
                    catch (GenericEntityException e) {
                    ...
                    if (techDataCalendarExcDay == null)    {
                        // Begin neogia specific : Use Neogia TechDataCalendar entities not ofbiz one, some there is some changes: entity and field name and type
                    	/*
                        ...
                        */
                        techDataCalendarExcDay = delegator.makeValue("CalendarException", null);
                        techDataCalendarExcDay.set("tdcalIdName", fixedAsset.get("calendarId"));
                        techDataCalendarExcDay.set("idNumber", delegator.getNextSeqId("CalendarException.idNumber") );
                        techDataCalendarExcDay.set("exceptionDate", exceptionDateStartTime);
                        techDataCalendarExcDay.set("usedCapacity",new Double(00.00));  // initialise to zero
                        techDataCalendarExcDay.set("capacity", fixedAsset.getDouble("productionCapacity").longValue());
                        // End neogia specific : Use Neogia TechDataCalendar entities not ofbiz one, some there is some minor changes (entity and field name)
...
//                                " Requested: " + workEffort.getString("quantityToProduce"), module);
                       ...
                       */     
                       if (newUsedCapacity.compareTo(techDataCalendarExcDay.getLong("capacity").doubleValue()) > 0)    {
                            String errMsg = "ERROR: fixed_Asset_sold_out AssetId: " + workEffort.get("fixedAssetId") + " on date: " + techDataCalendarExcDay.getString("exceptionDate");
                       // End neogia specific : Use Neogia TechDataCalendar entities not ofbiz one, some there is some minor changes (entity and field name)
                            Debug.logError(errMsg, module);
                            errorMessages.add(errMsg);
                            continue;
...
//                            " and for quantity: " + techDataCalendarExcDay.getDouble("usedCapacity").toString(), module);
                ...
                // Begin Neogia specific: FR#1706904 - Update fixedAsset Calendar - From MAAZZI
                updateFixedAssetCalendar(delegator, dispatcher, userLogin, locale, workEffort, fixedAsset, toBeStored, errorMessages, true);
                // End Neogia specific: FR#1706904 - Update fixedAsset Calendar
            }
        }
        if (errorMessages.size() > 0) {
            return ServiceUtil.returnError(errorMessages);
    ...
    public static void reserveInventory(GenericDelegator delegator, LocalDispatcher dispatcher, GenericValue userLogin, Locale locale, List orderItemShipGroupInfo, List dropShipGroupIds, Map itemValuesBySeqId, String orderTypeId, String productStoreId, List resErrorMessages) throws GeneralException {
        ...
        // Begin neogia specific : added purchase order reservation 
        //added orderstockeventplanned for each orderItemwhen orderTypeId is Purchase_Order            
        boolean receiveInventory = ("PURCHASE_ORDER".equals(orderTypeId) );
        String reserveServiceName = null;
        if (reserveInventory) reserveServiceName = "reserveStoreInventory";
        if (receiveInventory) reserveServiceName = "plannedReceiveStoreInventory";

        /*
        if (reserveInventory) {
        */
        if (reserveInventory || receiveInventory) {
            // END Neogia patch  : Purchase record
            // START inventory reservation
            ...
            if (orderItemShipGroupInfo != null && orderItemShipGroupInfo.size() > 0) {
                ...
                while (osiInfos.hasNext()) {
                    ...
                    if ("OrderItemShipGroupAssoc".equals(orderItemShipGroupAssoc.getEntityName())) {
                        ...
                        // Begin Neogia Specific : remove reservation only if orderItem is updated
                        /*
                        if ("ITEM_REJECTED".equals(itemStatus) || "ITEM_CANCELLED".equals(itemStatus) || "ITEM_COMPLETED".equals(itemStatus)) {
                        */
                        OrderItemShipGroup shipGroup = ((OrderItemShipGroupAssoc)orderItemShipGroupAssoc).getOrderItemShipGroup();
                        if (!("ITEM_CREATED".equals(itemStatus) ||"ITEM_APPROVED".equals(itemStatus) || (shipGroup != null && OrderItemStatus.APPROVED.equals(shipGroup.getStatusEnumId())))) {
                        // End Neogia Specific : remove reservation only if orderItem is updated
                        ...
                        // Begin Neogia specific: FR1706894 - RENTAL_ORDER_ITEM as a product items to be issued
                        /*
                        ...
                        */
                        ...
                        // End Neogia specific: FR1706894 - RENTAL_ORDER_ITEM as a product items to be issued
                            try {
                                ...
                                */
                                Double cancelQuantity = orderItemShipGroupAssoc.getDouble("cancelQuantity");
                                if (cancelQuantity == null) {
                                    cancelQuantity = new Double(0.0);
                                }
                                Double quantity = orderItemShipGroupAssoc.getDouble("quantity");
                                if (quantity == null) {
                                    quantity = new Double(0.0);
                                }
                                reserveInput.put("quantity",new Double( quantity.doubleValue()-cancelQuantity.doubleValue()));
                                // End JIRA 518 
                                reserveInput.put("userLogin", userLogin);
                                // Begin neogia specific : call service we have previously determined
                                /*
                                Map reserveResult = dispatcher.runSync("reserveStoreInventory", reserveInput);
								*/
                                Map reserveResult = dispatcher.runSync(reserveServiceName, reserveInput);
                                // End neogia specific : call service we have previously determined
...
//Begin Neogia Specific : BUG 1717653  Ofbiz recalculate orderadjustment service Fork
// this is the old method use in ofbiz but this method is necessary for neogia compatibility 
/*
...
*/
// Begin Neogia specific keep old ofbiz method recalcOrderShipping while perform all old call to this method
...
// End Neogia specific keep old ofbiz method recalcOrderShipping while perform all old call to this method
    public static Map recalcOrderTax(DispatchContext ctx, Map context) {
        LocalDispatcher dispatcher = ctx.getDispatcher();
        GenericDelegator delegator = ctx.getDelegator();
        String orderId = (String) context.get("orderId");
        ...
        // Neogia Modification we use tax on PurchaseOrder Too
        /*
        // don't charge tax on purchase orders
        if ("PURCHASE_ORDER".equals(orderHeader.getString("orderTypeId"))) {
            return ServiceUtil.returnSuccess();
        ...
        */
        ...
        // Begin Neogia Specific : check if orderHeader has tax exoneration
        /*
        if (shipGroups != null) {
        */
        if (shipGroups != null && !"Y".equals(orh.getNoTax())) {
        // End Neogia Specific : check if orderHeader has tax exoneration
            Iterator itr = shipGroups.iterator();
            while (itr.hasNext()) {
                GenericValue shipGroup = (GenericValue) itr.next();
                String shipGroupSeqId = shipGroup.getString("shipGroupSeqId");
    ...
    public static Map checkItemStatus(DispatchContext ctx, Map context) {
        ...
        */
        ...
        // Begin neogia specific :tache 196, traceur #529, remove test to call service checkOrderItemStatus
        if (orderItems != null) {
            Iterator itemIter = orderItems.iterator();
            ...
            int countItemProcessReci =0;
            int countItemProcessPrep =0;
            int countItemApproved =0;
            int countItemPrepared =0;
            int countItemSending =0;
            int countItemSent =0;
            int countReceived =0;
            int countItemCompleted =0;
            int countItemCancelled =0;
            ...
            while (itemIter.hasNext()) {
                ...
                if(statusMap.get(statusId)==null){
                    statusMap.put(statusId,new Integer(1));
                }
                else {
                    count = ((Integer)statusMap.get(statusId)).intValue();
                    statusMap.put(statusId,new Integer(1+count));
                }
                countTotal += 1;
            }
 
            //get cancelled item
            if(statusMap.get("ITEM_CANCELLED") != null)
                countCancelled += ((Integer)statusMap.get("ITEM_CANCELLED")).intValue();

            //get ITEM_PORCESS_RECI item
            if(statusMap.get("ITEM_PORCESS_RECI") != null)
                countItemProcessReci += ((Integer)statusMap.get("ITEM_PORCESS_RECI")).intValue();

            //get ITEM_PROCESS_PREP item
            if(statusMap.get("ITEM_PROCESS_PREP") != null)
                countItemProcessPrep += ((Integer)statusMap.get("ITEM_PROCESS_PREP")).intValue();

            //get ITEM_APPROVED item
            if(statusMap.get("ITEM_APPROVED") != null)
                countItemApproved += ((Integer)statusMap.get("ITEM_APPROVED")).intValue();

            //get PREPARED item
            if(statusMap.get("ITEM_PREPARED") != null)
                countItemPrepared += ((Integer)statusMap.get("ITEM_PREPARED")).intValue();

            //get SENDING item
            if(statusMap.get("ITEM_SENDING") != null)
                countItemSending += ((Integer)statusMap.get("ITEM_SENDING")).intValue();

            //get SENT item
            if(statusMap.get("ITEM_SENT") != null)
                countItemSent += ((Integer)statusMap.get("ITEM_SENT")).intValue();

            //get RECEIVED item
            if(statusMap.get("ITEM_RECEIVED") != null)
                countReceived += ((Integer)statusMap.get("ITEM_RECEIVED")).intValue();

            //get COMPLETED item
            if(statusMap.get("ITEM_COMPLETED") != null)
                countItemCompleted += ((Integer)statusMap.get("ITEM_COMPLETED")).intValue();

            //get cancelled item
            if(statusMap.get("ITEM_CANCELLED") != null)
                countItemCancelled += ((Integer)statusMap.get("ITEM_CANCELLED")).intValue();

           
            // find the next status to set to (if any)
           String newStatus = null;
           if(statusMap.get("ITEM_PORCESS_RECI") != null || ( countItemApproved != 0 && countReceived !=0)){
                newStatus = "ORDER_PROS_RECI";
            }
           else if(statusMap.get("ITEM_PROCESS_PREP") != null || ( countItemApproved != 0 && countItemPrepared !=0)){
               newStatus = "ORDER_PROS_PREP";
           }
           else if(statusMap.get("ITEM_APPROVED") != null && ((Integer)statusMap.get("ITEM_APPROVED")).intValue() == countTotal-countCancelled){
               newStatus = "ORDER_APPROVED";
           }
           else if(statusMap.get("ITEM_PREPARED") != null && (countItemApproved == 0 && countItemProcessPrep ==0 && countItemProcessReci ==0)){
               newStatus = "ORDER_PREPARED";
           }
           else if(statusMap.get("ITEM_SENDING") != null &&  (countItemApproved == 0 && countItemProcessPrep ==0 && countItemProcessReci ==0 && countItemPrepared == 0)){
        	   newStatus = "ORDER_SENDING";
           }
           else if(statusMap.get("ITEM_SENT") != null && (countItemApproved == 0 && countItemProcessPrep ==0 && countItemProcessReci ==0 && countItemPrepared == 0 && countItemSending == 0)){
               newStatus = "ORDER_SENT";
           }
           
           else if(statusMap.get("ITEM_RECEIVED") != null && (countItemApproved == 0 && countItemProcessPrep ==0 && countItemProcessReci ==0 && countItemPrepared == 0 && countItemSending == 0 && countItemSent == 0)){
        	   newStatus = "ORDER_RECEIVED";
           }
           
           else if(statusMap.get("ITEM_COMPLETED") != null&& (countItemApproved == 0 && countItemProcessPrep ==0 && countItemProcessReci ==0 && countItemPrepared == 0 && countItemSending == 0 && countItemSent == 0 && countReceived ==0)){
               newStatus = "ORDER_COMPLETED";
           }
           else if(statusMap.get("ITEM_CANCELLED") != null&& ((Integer)statusMap.get("ITEM_CANCELLED")).intValue()== countTotal){
               newStatus = "ORDER_CANCELLED";
           }
            
             
         // End neogia specific :tache 196, traceur #529, remove test to call service checkOrderItemStatus-->
            // now set the new order status
            if (newStatus != null) {
                Map serviceContext = UtilMisc.toMap("orderId", orderId, "statusId", newStatus, "userLogin", userLogin);
                Map newSttsResult = null;
    ...
    public static Map cancelOrderItem(DispatchContext ctx, Map context) {
        ...
        */
        ...
        // Begin Neogia Specific : BUG1491476
        // cancel one OISGA
        // find OISGA with orderId, orderItem ans shipGroupSeqId
        if (orderId != null && orderItemSeqId != null && shipGroupSeqId != null){
        	GenericValue OISGA = null;
        	try {
        		OISGA = delegator.findByPrimaryKey("OrderItemShipGroupAssoc", fields);
        	} catch (GenericEntityException e1) {
        		return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderErrorCannotGetOrderItemAssocEntity", UtilMisc.toMap("itemMsgInfo",itemMsgInfo), locale));
        	}
        	
        	Double OISGAQuantity = OISGA.getDouble("quantity");
        	
        	GenericValue orderItem = null;
        	try {
				orderItem = OISGA.getRelatedOne("OrderItem");
			} catch (GenericEntityException e) {
				return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderErrorCannotCancelItemItemNotFound", UtilMisc.toMap("itemMsgInfo",itemMsgInfo), locale));
			}
			Double orderItemQuantity = orderItem.getDouble("quantity");
        	Double orderItemCancelQuantity = orderItem.getDouble("cancelQuantity");
        	if (orderItemCancelQuantity == null)
        		orderItemCancelQuantity = new Double(0.0);
			Double newCancelQuantity = new Double(orderItemCancelQuantity.doubleValue() + OISGAQuantity.doubleValue());
			
        	// set cancelQuantity of orderItem
			orderItem.set("cancelQuantity", newCancelQuantity);
        	try {
				orderItem.store();
			} catch (GenericEntityException e) {
				return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderUnableToSetCancelQuantity", UtilMisc.toMap("itemMsgInfo",itemMsgInfo), locale));
			...
			// if orderItem quantity = orderItem cancelQuantity, change orderItem status to CANCELLED
        	BigDecimal quantityBD = new BigDecimal(orderItem.getDouble("quantity").doubleValue()).setScale(decimals, rounding);
        	BigDecimal cancelQuantityBD = new BigDecimal(orderItem.getDouble("cancelQuantity").doubleValue()).setScale(decimals, rounding);
        	
        	if (quantityBD.compareTo(cancelQuantityBD) == 0){
        		Map statusCtx = UtilMisc.toMap("orderId", orderId, "orderItemSeqId", orderItemSeqId, "statusId", "ITEM_CANCELLED", "userLogin", userLogin);
				try {
        	...
        	// remove OISGA
        	Map result = OrderItemShipGroupAssocServices.deleteOrderItemShipGroupAssoc(ctx, context);
        	if (ServiceUtil.isError(result))
        		return result;
            
            //perform recaculate of all adjustment : in a complete cancel those aservices 
            //are calling by seca on changeOrderItemStatus and if new statut is cancelled
            context.remove("orderItemSeqId");
            context.remove("shipGroupSeqId");
            try{
                dispatcher.runSync("recalcShippingTotal", context);
                dispatcher.runSync("recalcTaxTotal", context);
                dispatcher.runSync("resetGrandTotal", context);
                dispatcher.runAsync("sendOrderChangeNotification", context, true);
            } catch (GenericServiceException e) {
                return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderUnableToSetCancelQuantity", UtilMisc.toMap("itemMsgInfo",itemMsgInfo), locale));  
            }
        }
        // End Neogia Specific : BUG1491476
        
        // Begin Neogia Specific : BUG1491476
        else if (shipGroupSeqId == null){ 
            ...
            if( orderItems != null ){
                for(OrderItem orderItem : orderItems) {
                    ...
                    // cancel all quantity --> set cancelQuantity of orderItem with quantity
                    orderItem.set("cancelQuantity", orderItemQuantity);
                    try {
                        orderItem.store();
                    } catch (GenericEntityException e1) {
                        return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderUnableToSetCancelQuantity", UtilMisc.toMap("itemMsgInfo",itemMsgInfo), locale));
                    ...
                    // change item status to canceled
                    Map statusCtx = UtilMisc.toMap("orderId", orderId, "orderItemSeqId", orderItemSeqId, "statusId", "ITEM_CANCELLED", "userLogin", userLogin);
                    try {
                        dispatcher.runSync("changeOrderItemStatus", statusCtx);
                    } catch (GenericServiceException e) {
                    ...
                    // set cancelQuantity of OISGAs associated with orderitem
                    List OISGAs = null;
                    try {
                        OISGAs = orderItem.getRelated("OrderItemShipGroupAssoc");
                    } catch (GenericEntityException e) {
                        return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderErrorCannotGetOrderItemAssocEntity", UtilMisc.toMap("itemMsgInfo",itemMsgInfo), locale));
                    }

                    if (OISGAs != null && !OISGAs.isEmpty()){
                        Iterator i = OISGAs.iterator();
                        while (i.hasNext()) {
                            GenericValue orderItemShipGroupAssoc = (GenericValue) i.next();

                            // remove OISGA
                            context.put("shipGroupSeqId", orderItemShipGroupAssoc.getString("shipGroupSeqId"));
                            context.put("orderItemSeqId", orderItem.getOrderItemSeqId());
                            Map result = OrderItemShipGroupAssocServices.deleteOrderItemShipGroupAssoc(ctx, context);
                            if (ServiceUtil.isError(result)){
    ...
    public static Map setItemStatus(DispatchContext ctx, Map context) {
        ...
        // Begin Neogia Specific : FR1463456
        LocalDispatcher dispatcher = ctx.getDispatcher();
        // End Neogia Specific : FR1463456
        ...
        // Begin Neogia FR1485734 : Credit Limit on purchase Order
        // get the type of the order
        OrderHeader orderHeader = OrderHeaderServices.findByPrimaryKey(delegator,orderId);
         
        if ( orderHeader != null && OrderStatus.CREATED.equals( orderHeader.getStatusEnumId() ) && orderHeader.isPurchaseOrder() ){
            BigDecimal grandTotal = new BigDecimal(orderHeader.getGrandTotal());
            
            // Get the creditLimit
            UserLogin ul = (UserLogin) userLogin;
            Party party = ul.getParty();
            
            if(org.ofbiz.party.party.developed.PartyWorker.isOrderAmountOver(party, grandTotal, OrderType.PURCHASE))            
                    return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderApprobationTotalPurchaseOrderError",locale));     
        }
        // End Neogia FR1485734
...
//              Begin neogia specific : tracker 655 : get orderId from changeorderItemStatus for quickPorcessOrder-->
                if(!orderItem.getString("statusId").equals("ITEM_CANCELLED")){
//              Begin neogia specific : tracker 655 : get orderId from changeorderItemStatus for quickPorcessOrder-->
            ...
            if (toBeStored.size() > 0) {
                ...
                // Begin Neogia Specific : BUG1502264
                // if orderItem is approved or processing_prepare and orderHeader is prepared --> change orderHeader status to processing_prepare
                if ("ORDER_PREPARED".equals(orderHeader.getStatusEnumId())){
                	if ("ITEM_APPROVED".equals(orderItem.getString("statusId")) || "ITEM_PROCESS_PREP".equals(orderItem.getString("statusId"))){
                		Map ctxOrderHeader = UtilMisc.toMap("orderId", orderId, "statusId", "ORDER_PROS_PREP", "userLogin", userLogin);
                		Map result = null;
                		try {
                			result = dispatcher.runSync("changeOrderStatus", ctxOrderHeader);
                		} catch (GenericServiceException e) {
                			Debug.logError(e, "Problem calling the changeOrderStatus service", module);
                		}
                		if (ServiceUtil.isError(result)) {
                ...
                // End Neogia SPecific : BUG1502264
                
                // determine the new status of OISG in function of orderItem status
                String OISGStatus = null;
            	if ("ITEM_APPROVED".equals(statusId))
            		OISGStatus = "ITEM_APPROVED";
            	// Begin Neogia Specific : BUG1513400
            	else if ("ITEM_CANCELLED".equals(statusId))
            		OISGStatus = "ITEM_CANCELLED";
            	// End Neogia Specific : BUG1513400
            	// Begin Neogia Specific : BUG1491476
            	// don't change OISG status to canceled when orderItem status is canceled because OISG can be associated with another orderItem 
            	/*
            	if ("ITEM_CANCELLED".equals(statusId))
            		OISGStatus = "ITEM_CANCELLED";
            	*/
            	...
            	if (OISGStatus != null){
            		// update OISG status in function of statusId
            		List OISGAssoc = null;
            		Map orderItemMap = UtilMisc.toMap("orderId", orderItem.getString("orderId"), "orderItemSeqId", orderItem.getString("orderItemSeqId"));
            		try {
            			OISGAssoc = delegator.findByAnd("OrderItemShipGroupAssoc",orderItemMap);
            			if(Debug.verboseOn())Debug.logVerbose("list OISGAssoc : "+OISGAssoc, resource);
            		} catch (GenericEntityException e) {
            			return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderErrorCannotStoreStatusChanges", locale) +e.getMessage());
            		}
            		if (OISGAssoc != null && !OISGAssoc.isEmpty()) {
            			Iterator OISGAssocIterator = OISGAssoc.iterator();
            			OrderItemShipGroup OISG = null;
            			while (OISGAssocIterator.hasNext()) {
            				GenericValue gvOISGAssoc = (GenericValue) OISGAssocIterator.next();
            				String shipGroupSeqId = gvOISGAssoc.getString("shipGroupSeqId");
            				Map OISGAssocMap = UtilMisc.toMap("orderId", orderId, "shipGroupSeqId", shipGroupSeqId);
            				try {
            					OISG = (OrderItemShipGroup)delegator.findByPrimaryKey("OrderItemShipGroup",OISGAssocMap);
            					if(Debug.verboseOn())Debug.logVerbose("list OISG : "+OISG, resource);
            				} catch (GenericEntityException e) {
            					return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderErrorCannotStoreStatusChanges", locale) +e.getMessage());
            				}

            				// Begin Neogia Specific : BUG1513400
            				// update OISG status to cancelled only if all orderItems associated with OISG are cancelled
            				if (OISGStatus.equals("ITEM_CANCELLED")){
            					if (OrderItemShipGroupAssocViewQuery.findCountByConditionOIStatusIdAndOrderItemShipGroupAndOrder(delegator, orderId, shipGroupSeqId, statusId) == 0){
            						//OISGStatus = "ITEM_CANCELLED";
            						
            						// update OISG status
            						if (! OISG.getString("statusId").equals(OISGStatus)){
            							try{
            								Map ctxOISG = UtilMisc.toMap("orderId", orderId, "shipGroupSeqId", shipGroupSeqId, "newStatusId", OISGStatus, "userLogin", context.get("userLogin"), "locale", locale);
            								dispatcher.runSync("changeOISGStatus",ctxOISG); 
            							}catch(GenericServiceException e){
            								return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderErrorCannotStoreStatusChanges", locale) +e.getMessage());
            							}
            						}
            					...
            					//update if this OISG has OSEP and if all OSEP are not in a shipment
            					long inShipmentOSEP = OrderStockEventPlannedQuery.CountByAndOISGAndShipmentNotEmpty(delegator, orderId, shipGroupSeqId);
            					List OSEPs = OrderStockEventPlannedQuery.findByAndOISG(delegator, orderId, shipGroupSeqId);
            					if( !(inShipmentOSEP > 0) && (OSEPs != null && !OSEPs.isEmpty())){
        ...
        // Begin neogia specific : tracker 517 : get orderId from changeorderItemStatus for quickPorcessOrder
        /*
        return ServiceUtil.returnSuccess();
        */
        ...
        return result;
        // End neogia specific : tracker 517 : get orderId from changeorderItemStatus for quickPorcessOrder
    }
    ...
    /** Begin Neogia specific : add shipment sheduled mail , jira send 
       Service to email all customer with shipment associate to their order is scheduled */
    public static Map sendShipmentScheduledNotification(DispatchContext ctx, Map context) {
        ...
        try {
            ...
            Debug.logError(e, "Problem getting Shipment", module);
        }
        if (shipment == null)
            return ServiceUtil.returnFailure("Could not find shipment ID [" + shipmentId + "]");

        //send mail to all cutomer's orders
        String previousOrder = "";
        Map result = null;
        try{
            for( GenericValue orderShipment : (List<GenericValue>) 
                    shipment.getRelatedCache("OrderShipment", UtilMisc.toList("orderId")) )
                if( ! previousOrder.equals( orderShipment.getString("orderId") ) ){
                    
                    GenericValue orderRole = delegator.findByPrimaryKeyCache("OrderRole", 
                            UtilMisc.toMap("orderId", orderShipment.getString("orderId"),
                                    "partyId", shipment.getString("partyIdTo"),
                                    "roleTypeId", "BILL_TO_CUSTOMER ") );
                    if( orderRole != null )  //don't send email if the customer is the same of shipper
                        continue;
                    
                    context.put( "orderId", orderShipment.getString("orderId") );
                    result = sendOrderNotificationScreen(ctx, context, "PRDS_SHIP_SCHEDULE");
                    if( ServiceUtil.isError(result) )
                        return result;
                    previousOrder = orderShipment.getString("orderId");
                }                              
        }
        catch( GenericEntityException e ){
        ...
        List partyAndContactMechs = null;
        try{ 
            partyAndContactMechs = delegator.findByAndCache("PartyAndContactMech",         
                UtilMisc.toMap( "partyId", shipment.getString("partyIdTo"),
                        "contactMechTypeId", "EMAIL_ADDRESS" ) );
        }
        catch( GenericEntityException e ){
            return ServiceUtil.returnError( e.toString() );
        }
        partyAndContactMechs = EntityUtil.filterByDate( partyAndContactMechs, UtilDateTime.nowDate() );
        if( partyAndContactMechs != null ){
            context.put( "sendTo", EntityUtil.getFirst(partyAndContactMechs) );
            //use primary order productstore to found email screen et adress to use. 
            context.put( "orderId", shipment.getString("primaryOrderId") );
            return sendOrderNotificationScreen(ctx, context, "PRDS_SHIP_SCHEDULE");            
        }
        return ServiceUtil.returnSuccess();
    }    
    //End
    protected static Map sendOrderNotificationScreen(DispatchContext dctx, Map context, String emailType) {
        ...
        //Begin Neogia specific : add shipment sheduled mail , jira send
        if( context.containsKey("shipmentId") ) {
            bodyParameters.put("shipmentId", context.get("shipmentId"));
        }
        //end
    ...
    public static Map createOrderNote(DispatchContext dctx, Map context) {
        ...
        // Begin Neogia Specific : FR1517486
        String footerNote = (String) context.get("footerNote");
        // End Neogia Specific : FR1517486
        ...
        try {
            ...
            // Set the order info        	// Begin Neogia Specific : FR1517486
        	/* 
            Map fields = UtilMisc.toMap("orderId", orderId, "noteId", noteId, "internalNote", internalNote);
            */
        	Map fields = UtilMisc.toMap("orderId", orderId, "noteId", noteId, "internalNote", internalNote, "footerNote", footerNote);
        	// End Neogia Specific : FR1517486
    ...
    // Begin Neogia Specific : FR1518032
    /** Service to remove a order header note. */
    public static Map modifyOrderNote(DispatchContext dctx, Map context) {
        GenericDelegator delegator = dctx.getDelegator();
        Locale locale = (Locale) context.get("locale");
        ...
        boolean boolInternalNote = internalNote !=null && "Y".equalsIgnoreCase(internalNote) ? true : false;
        boolean boolFooterNote = footerNote !=null && "Y".equalsIgnoreCase(footerNote) ? true : false;
        
        if (noteId == null)
        	return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderNoOrderHeaderNoteFound", locale));
        
        // find and update noteDate with noteId
        NoteData noteData = NoteDataServices.findByPrimaryKey(delegator, noteId);
        noteData.setNoteInfo(noteInfo);
		try {
			noteData.store();
		} catch (GenericEntityException e) {
			return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderCannotStoreNoteData", locale));
		}
		
        // find and update note with noteId and orderId
        OrderHeaderNote note = OrderHeaderNoteServices.findByPrimaryKey(delegator, noteId, orderId);
        note.setInternalNote(boolInternalNote);
        note.setFooterNote(boolFooterNote);
        try {
			note.store();
		} catch (GenericEntityException e) {
			return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderCannotStoreOrderHeaderNote", locale));
		}
    ...
    /** Service to remove a order header note. */
    public static Map removeOrderNote(DispatchContext dctx, Map context) {
        GenericDelegator delegator = dctx.getDelegator();
        Locale locale = (Locale) context.get("locale");
        ...
        // find noteDate with noteId
        NoteData noteData = NoteDataServices.findByPrimaryKey(delegator, noteId);
        
        // find orderHeaderNote with noteId and orderId
        OrderHeaderNote note = OrderHeaderNoteServices.findByPrimaryKey(delegator, noteId, orderId);
        
        // remove note and noteData
        try {
			note.remove();
		} catch (GenericEntityException e) {
			return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderErrorCannotRemoveOrderHeaderNote", locale));
		}
		
		try {
			noteData.remove();
		} catch (GenericEntityException e) {
			return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderErrorCannotRemoveNoteData", locale));
		}
		
    	return ServiceUtil.returnSuccess();
    }
    // End Neogia Specific : FR1518032
    ...
    // Begin neogia specific : order modification feature
    /* allow modification of an order item after its creation */
        public static Map updateOrderItem(DispatchContext ctx, Map context) {
            GenericDelegator delegator = ctx.getDelegator();
            LocalDispatcher dispatcher = ctx.getDispatcher();        
            GenericValue userLogin = (GenericValue) context.get("userLogin");
            String orderId = (String) context.get("orderId");
            ...
            // check and make sure order item exists and can be modified
            GenericValue orderHeader = null;
            GenericValue orderItem = null;
            try {
                Map orderItemPK = UtilMisc.toMap("orderId", orderId, "orderItemSeqId", orderItemSeqId);
                orderItem = delegator.findByPrimaryKey("OrderItem", orderItemPK);
                if (orderItem != null)
            ...
            if (orderItem == null || orderHeader == null)
                return ServiceUtil.returnError("ERROR: OrderItem and/or OrderHeader don't exist");
    
            String orderItemStatusId = (String) orderItem.get("statusId");
            if (!("ITEM_CREATED".equals(orderItemStatusId) || "ITEM_APPROVED".equals(orderItemStatusId)))
                return ServiceUtil.returnError("Error: OrderItem can't be modified");
            
            
            // update quantity
            if (context.get("remainingQuantity") != null) {
                double remainingQuantity = ((Double)context.get("remainingQuantity")).doubleValue();
                double quantity = ((Double)orderItem.get("quantity")).doubleValue();
                double cancelQuantity = 0.0;
                
                if (orderItem.get("cancelQuantity") != null)
                    cancelQuantity = ((Double)orderItem.get("cancelQuantity")).doubleValue();
                
                if (remainingQuantity < (quantity-cancelQuantity)) {
                    Double quantityToCancel = new Double(quantity-cancelQuantity-remainingQuantity);
                    
                    // update order item cancel quantity
                    try {
                        orderItem.set("cancelQuantity", new Double(quantity-remainingQuantity));
                        orderItem.store();
                    } catch (GenericEntityException e) {
                        Debug.logError(e, module);
                        return ServiceUtil.returnError("Unable to update cancel quantity of item");
                    }
                    
                    // cancel inventory reservation
                    try {
                        Map serviceCtx = UtilMisc.toMap("orderId", orderId, "orderItemSeqId", orderItemSeqId,
                                                        "userLogin", context.get("userLogin"),
                                                        "cancelQuantity", quantityToCancel);
                        //Neogia patch
    					//added if Purchase order then run service to update orderstockeventplanned
    					String orderTypeId = orderHeader.getString("orderTypeId");
    					
    					// if sales order
    					if (("SALES_ORDER".equals(orderTypeId) || "WORK_ORDER".equals(orderTypeId))
    							&& orderItem.get("productId") != null){
    						Map results = dispatcher.runSync("cancelOrderItemInvResQty", serviceCtx);
    					...
    					if (("PURCHASE_ORDER".equals(orderTypeId) )
    							&& orderItem.get("productId") != null){
    						Map results = dispatcher.runSync("cancelOrderItemInvRecievQty", serviceCtx);
                        ...
                        Debug.logError(e, module);
                        return ServiceUtil.returnError("Unable to cancel item inventory reservation");
                    }
                    
                    // update order item status
                    if (remainingQuantity == 0.0) {
                        try {
                            Map cancelCtx = UtilMisc.toMap("orderId", orderId, "orderItemSeqId", orderItemSeqId, "userLogin", context.get("userLogin"), "fromStatusId", orderItemStatusId, "statusId", "ITEM_CANCELLED");
                            Map results = dispatcher.runSync("changeOrderItemStatus", cancelCtx);
                            if (ServiceUtil.isError(results))
                                return results;
                        } catch (GenericServiceException e) {
                            return ServiceUtil.returnError("Unable to cancel order Item : " + e.getMessage());
                        }
                    }
                    
                    needRecalc = true;
                } else if (remainingQuantity > (quantity-cancelQuantity)) {
    
                    String orderTypeId = orderHeader.getString("orderTypeId");
                    String productStoreId = orderHeader.getString("productStoreId");
                    
                    // only reserve product items in sales or work order; ignore non-product items
                    if (("SALES_ORDER".equals(orderTypeId) || "WORK_ORDER".equals(orderTypeId))
                        && orderItem.get("productId") != null) {
                        try {
                            Map reserveResult = dispatcher.runSync("reserveStoreInventory", UtilMisc.toMap(
                                ...
                                "quantity", new Double(remainingQuantity-(quantity-cancelQuantity)), "orderId", orderItem.getString("orderId"), 
                                "orderItemSeqId", orderItem.getString("orderItemSeqId"), "userLogin", userLogin));
                                
                            if (ServiceUtil.isError(reserveResult))
                               return reserveResult;
                                
                            double quantityNotReserved = 0.0;
                            if (reserveResult.get("quantityNotReserved") != null)
                                quantityNotReserved = ((Double)reserveResult.get("quantityNotReserved")).doubleValue();
                            
                            if (quantityNotReserved > 0.0)
                                return ServiceUtil.returnError("Unable to reserve inventory");
                    ...
                    //Neogia patch 
    				// update orderstockeventplanned when orderitem is updated
    				if (("PURCHASE_ORDER".equals(orderTypeId) )
    						&& orderItem.get("productId") != null) {
    					try {
    						String nullString = null;
    						Map reserveResult = dispatcher.runSync("plannedReceiveStoreInventory", UtilMisc.toMap(
    								"productStoreId", productStoreId, "productId", orderItem.getString("productId"),
    								"quantity", new Double(remainingQuantity-(quantity-cancelQuantity)), "orderId", orderItem.getString("orderId"), 
    								"orderItemSeqId", orderItem.getString("orderItemSeqId"), "userLogin", userLogin));
                    ...
                    try {
                        orderItem.set("quantity", new Double(remainingQuantity+cancelQuantity));
                        orderItem.store();
                    } catch (GenericEntityException e) {
                        Debug.logError(e, module);
                        return ServiceUtil.returnError("Unable to update ordered quantity of item");
                    }
            ...
            // update unit price
            if (context.get("unitPrice") != null) {
                double newUnitPrice = ((Double)context.get("unitPrice")).doubleValue();
    
                if (newUnitPrice < 0.0)
                    return ServiceUtil.returnError("An unit price can't be lesser than zero");
                
                try {
                    orderItem.refresh();
                    orderItem.set("unitPrice", new Double(newUnitPrice));
                    orderItem.store();
                } catch (GenericEntityException e) {
                    Debug.logError(e, module);
                    ServiceUtil.returnError("Unable to update unit price of item");
                }
                
                needRecalc = true;
            }
            
            // update item's estimated delivery date
            if (context.get("estimatedDeliveryDate") != null) {
                Timestamp newEstimatedDeliveryDate = (Timestamp)context.get("estimatedDeliveryDate");
                if (!newEstimatedDeliveryDate.equals((Timestamp)orderItem.get("estimatedDeliveryDate"))) {
                    try {
                        orderItem.refresh();
                        orderItem.set("estimatedDeliveryDate", newEstimatedDeliveryDate);
                        orderItem.store();                        
                        
                        //Begin Neogia specific : update Orderstockeventplanned
                       Map requestParameters = UtilMisc.toMap(
                               "orderId", orderItem.getString("orderId"),
                               "orderItemSeqId", orderItem.getString("orderItemSeqId"),
                               "estimatedDeliveryDate", (Date)newEstimatedDeliveryDate,
                               "userLogin", userLogin);
						 dispatcher.runSync("updateEstimatedDeliveryDate", requestParameters);
                        //En neogia specific :  update Orderstockeventplanned 
                    } catch (GenericEntityException e) {
                        Debug.logError(e, module);
                        ServiceUtil.returnError("Unable to update item's estimated delivery date");
                    }    					 
                    catch (GenericServiceException e) {
						String errMsg = "Fatal error calling updateEstimatedDeliveryDate service: " + e.toString();
						Debug.logError(e, errMsg, module);
						return ServiceUtil.returnError(errMsg);
                    }
            ...
            // update item's comments
            if (context.get("comments") != null) {
                String comments = (String) context.get("comments");
                if (!comments.equals(orderItem.getString("comments"))) {
                    try {
                        orderItem.refresh();
                        ...
                        Debug.logError(e, module);
                        ServiceUtil.returnError("Unable to update comments of item");
                    }
                }
            }
            
            if (needRecalc) {
                Map serviceCtx = UtilMisc.toMap("orderId", orderId, "userLogin", context.get("userLogin"));
                Map results = null;
                try {
                    String orderTypeId = orderHeader.getString("orderTypeId");
                    if (!"PURCHASE_ORDER".equals(orderTypeId) ){
                        results = dispatcher.runSync("recalcShippingTotal", serviceCtx);
                    if (ServiceUtil.isError(results))
                       return ServiceUtil.returnError(null, null, null, results);
       				}
                    results = dispatcher.runSync("recalcTaxTotal", serviceCtx);
                    if (ServiceUtil.isError(results))
                       return ServiceUtil.returnError(null, null, null, results);
                    
                    results = dispatcher.runSync("resetGrandTotal", serviceCtx);
                    if (ServiceUtil.isError(results))
                       return ServiceUtil.returnError(null, null, null, results);
                    
                } catch (GenericServiceException e) {
                    return ServiceUtil.returnError("Unable to reset order grand total " + e.getMessage());
                }
            }
            
            return ServiceUtil.returnSuccess();
        }
    // End neogia specific : order modification feature 

        //      on standby of Jira OFBIZ-369 on gestion of UOM quantity 
    public static Map addItemToApprovedOrder(DispatchContext dctx, Map context) {
        ...
        // Begin Neogia specific
        Double uomQuantityDb = (Double) context.get("quantity");
        String itemQuantityUomId=(String) context.get("itemQuantityUomId");//on standby of Jira OFBIZ-369 on gestion of UOM quantity
        // End Neogia specific
        Double quantity = (Double) context.get("quantity");
        Double amount = (Double) context.get("amount");
        String overridePrice = (String) context.get("overridePrice");
        // Begin Neogia specific perhaps an old ofbiz code
        Timestamp itemDesiredDeliveryDate =(Timestamp) context.get("itemDesiredDeliveryDate");
        String itemComment = (String) context.get("itemComment");
        // End Neogia specific an old ofbiz code
        
        // Begin Neogia Specific : FR#1374361 Added new variable to add Invoice attribute and package attribute
        Double invoicingQuantity = (Double) context.get("invoicingQuantity");
        String invoicingQtyUomId =(String) context.get("invoicingQtyUomId");
        Double quantityPack = (Double) context.get("quantityPack");
        String packaging=(String) context.get("packaging");            
        // End Neogia Specific : FR#1374361
        ...
        int shipGroupIdx = -1;
// Begin Neogia Specific : create a new ship group is shipGroupSeqId empty
        boolean createShipGroup = false;
       if(UtilValidate.isEmpty(shipGroupSeqId))
           createShipGroup = true;
       
       if(! createShipGroup){
//End Neogia Specific :   create a new ship group is shipGroupSeqId empty     
        try {
...
//      End Neogia Specific :   create a new ship group is shipGroupSeqId empty     
        ...
        //Begin Neogia specific Complete comment
        // obtain a shopping cart object for updating
        ShoppingCart cart = null;
        try {
            cart = loadCartForUpdate(dispatcher, delegator, userLogin, orderId);
        ...
        //End Neogia specific Complete comment


        //  Begin Neogia Specific : add uom price and quantity management + on standby of Jira OFBIZ-369 on gestion of UOM quantity 
        double uomQuantity=uomQuantityDb.doubleValue();
        //get quantity corresponding to the default uom
        if(uomQuantity>0)
        {
                quantity=new Double(uomQuantity);
//              Begin Neogia Specific : FR#1374361 if we are in screen packaging then we don't do convertion but we save item attributes
                /* old code
                if(itemQuantityUomId!=null )
                */
                if(UtilValidate.isEmpty(itemQuantityUomId)){
                    Product product = ProductServices.findByPrimaryKey(delegator,productId);
                    itemQuantityUomId = product.getStockUomId();
                }
                
                if(itemQuantityUomId!=null && invoicingQuantity==null)
//                  End Neogia Specific : FR#1374361
                {
                    String uomId=itemQuantityUomId;
                    String uomIdTo=null;
                   if(productId!=null)
                    {
                        GenericValue product=null;
                        ...
                        if(uomId!=null&&product!=null)
                        {
                           //check a product must note have an quantityUom if he hasn't a default Uom
                            //Begin Neogia Specific : add Uom and price management
                            /*
                           ...
                           */
                           ...
                           {
                               return ServiceUtil.returnError( UtilProperties.getMessage(resource_error,"OrderErrorCannotHaveUomWithoutDefaultUom", locale));
                           }
                           try {
                               ...
                               Debug.log("originalValue :"+ uomQuantity, module);
                               ...
                               if (ServiceUtil.isError(resultUom))
                               {
                                   ...
                                   return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderErrorCouldNotFindConversion", msgEr, locale));
                               }
                               if(resultUom.get("convertedValue")!=null){
                                    quantity = (Double)resultUom.get("convertedValue");
                               }
                                }
 ...
 //Begin NEogia specific : add uom management
        Map attributes = FastMap.newInstance();
            if(UtilValidate.isEmpty(itemQuantityUomId)){
                Product product = ProductServices.findByPrimaryKey(delegator, productId);
                if (product != null)
                    attributes.put("itemQuantityUomId", product.getStockUomId());
            }
            else
                attributes.put("itemQuantityUomId", itemQuantityUomId);
 //End NEogia specific : add uom management
 // Begin Neogia specific To comment           
            boolean resetShipGroup = false;
            attributes.put("resetShipGroup", String.valueOf(resetShipGroup));
// End Neogia specific
                    

        // add in the new product
        try {
            // Begin neogia specific : add attributes
            /*
            ...
            */
            ShoppingCartItem item = ShoppingCartItem.makeItem(null, productId, null, quantity.doubleValue(), null, null, null, null, null, null, null, attributes, prodCatalogId, null, null, null, dispatcher, cart, null, null , null, Boolean.FALSE, Boolean.FALSE);
            // End neogia specific : add attributes
//          Begin Neogia Specific : FR#1374361 if we are in screen packaging then we don't do convertion but we save item attributes
            if(invoicingQuantity!=null) {        
            	item.setAttribute("quantityPack",quantityPack.toString());
            	item.setAttribute("packaging",packaging);
            	item.setAttribute("invoicingQuantity", invoicingQuantity.toString()); 
            	item.setAttribute("invoicingQtyUomId", invoicingQtyUomId); 
            }
//              End Neogia Specific : FR#1374361
            if (basePrice != null&&overridePrice!=null) {
                ...
                // Begin neogia specific : avoid reset of shipgroup
                /*
                item.setQuantity(quantity.doubleValue() + 1, dispatcher, cart, false);
                item.setQuantity(quantity.doubleValue(), dispatcher, cart, false);
                */
                item.setQuantity(quantity.doubleValue() + 1, dispatcher, cart, false, false);
                item.setQuantity(quantity.doubleValue(), dispatcher, cart, false, false);
                // End neogia specific : avoid reset of shipgroup
                item.setBasePrice(basePrice.doubleValue());
                item.setIsModifiedPrice(true);
            }

            // Begin Neogia specific To complete comment
            // set desiredDeliveryDate
            item.setDesiredDeliveryDate(itemDesiredDeliveryDate);
            
            // set item comment
            item.setItemComment(itemComment);
            // End Neogia specific To complete comment
            
            // Begin Neogia Specific : FR1445562
            /*
            //Begin Neogia Specific : set status to Approved and not to created
            item.setStatusId("ITEM_APPROVED");
            //End Neogia Specific : set status to Approved and not to created
            */
            // End Neogia Specific : FR1445562
            ...
            // Set item status in function of the order status
            if (orderHeader.getString("statusId").equals("ORDER_CREATED")){
            ...
            //Begin Neogia Specific : add new ShipGroup if it doesn't exist
            //get last shipgroup index 
            if(createShipGroup){
                int shipGroupIndex = cart.getShipGroupSize() ;
                CartShipInfo csi = cart.getShipInfo(shipGroupIndex - 1 );
                
                //create a new Shipinfo
                CartShipInfo csi2 = cart.addNewShipInfo();
                shipGroupIndex = cart.getShipGroupSize() - 1 ;
               
                //fullfil shipInfo
                cart.setShipAfterDate(shipGroupIndex, csi.shipAfterDate);
                cart.setShipBeforeDate(shipGroupIndex, csi.shipBeforeDate);
                cart.setShipmentMethodTypeId(shipGroupIndex, csi.shipmentMethodTypeId);
                cart.setCarrierPartyId(shipGroupIndex, csi.carrierPartyId);
                cart.setMaySplit(shipGroupIndex, new Boolean(csi.maySplit));
                cart.setGiftMessage(shipGroupIndex, csi.giftMessage);
                cart.setShippingContactMechId(shipGroupIndex, csi.contactMechId);
                cart.setShippingInstructions(shipGroupIndex, csi.shippingInstructions);
                cart.setItemShipGroupQty(item, item.getQuantity(), shipGroupIndex);
                cart.setEstimatedDeliveryDate(shipGroupIndex, itemDesiredDeliveryDate);
                cart.setEstimatedShipDate(shipGroupIndex, itemDesiredDeliveryDate);
                cart.setClientDeliveryDate(shipGroupIndex, itemDesiredDeliveryDate);
                cart.setStatusId(shipGroupIndex, item.getStatusId());
            }
            else
            //End Neogia Specific : add new ShipGroup if it doesn't exist
            // set the item in the selected ship group
            // Begin Neogia Specific : set item quantity in the right shipInfo
            {
                shipGroupIdx = cart.getShipInfoIndex(shipGroupSeqId);
            // End Neogia Specific : set item quantity in the right shipInfo
        ...
        // log an order note
        try {
        	// Begin Neogia specific add footernote parameter
        	/*
            ...
            */
            dispatcher.runSync("createOrderNote", UtilMisc.toMap("orderId", orderId, "note", "Added item to order: " +
                    productId + " (" + quantity + ")", "internalNote", "Y", "footerNote", "", "userLogin", userLogin));
            // End Neogia specific add footernote parameter
        } catch (GenericServiceException e) {
            Debug.logError(e, module);
        }
    ...
    public static Map updateApprovedOrderItems(DispatchContext dctx, Map context) {
        ...
        Locale locale = (Locale) context.get("locale");
        String orderId = (String) context.get("orderId");
        Map itemDesiredDeliveryDateMap = (Map) context.get("itemDesiredDeliveryDateMap");
        ...
        // Begin Neogia specific: FR#1706904 - Get rental info from the context
        Map reservStartMap = (Map) context.get("reservStartMap"); 
        Map reservLengthMap = (Map) context.get("reservLengthMap");
        Map reservPersonsMap = (Map) context.get("reservPersonsMap");
        // End Neogia specific: FR#1706904 - Get rental info from the context
//Begin Neogia Specific : add comment item
        Map itemCommentMap = (Map) context.get("itemCommentMap");
//End Neogia Specific : add comment item
//Begin Neogia Specific : fix edit OI with multi OISGA
        Map notFoundOISGAMap = FastMap.newInstance();
//End Neogia Specific : fix edit OI with multi OISGA        
//      Begin Neogia Specific : FR#1374361 if we are in screen packaging then we read attributes 
        Map quantityPackMap = (Map) context.get("quantityPackMap");
        Map packagingMap = (Map) context.get("packagingMap");
        Map quantityInvoiceMap = (Map) context.get("quantityInvoiceMap"); 
//      End Neogia Specific : FR#1374361

        // Begin Neogia specific: FR#1706904 - logInfo
        Debug.logInfo("reservStartMap" + reservStartMap, module);
        // Begin Neogia specific: FR#1706904 - logInfo
        // obtain a shopping cart object for updating
        ShoppingCart cart = null;
        try {
        ...
        while (i.hasNext()) {
            ...
            */
            try {
              groupQty = ((Double) ObjectType.simpleTypeConvert(quantityStr, "Double", null, locale, true )).doubleValue();    
            } catch (GeneralException e) {
                Debug.logError(e, module);
                return ServiceUtil.returnError(e.getMessage());
            }
...
//          Begin Neogia Specific : FR#1374361 if we are in screen packaging then we split with "__": name with ":" is not a valid html name
            boolean pack = false;
            if(itemInfo.length<2)
            {
            	itemInfo = key.split("__");
            	pack = true;
            }
...
//              Begin Neogia Specific : FR#1374361 if we are in screen packaging then we don't do convertion but we save item attributes
                /* old code
                if(cartItem.getQuantityUomId()!=null&&cartItem.getProductId()!=null)
                */
                if(pack)
                {                    	
                	cartItem.getAttributes().put("quantityPack",(String)quantityPackMap.get(itemInfo[0]));
                	cartItem.getAttributes().put("packaging",(String)packagingMap.get(itemInfo[0]));
                	cartItem.getAttributes().put("invoicingQuantity", (String)quantityInvoiceMap.get(itemInfo[0]));                	                	
                }
                if(cartItem.getQuantityUomId()!=null&&cartItem.getProductId()!=null&&!pack)
//                  End Neogia Specific : FR#1374361
                {
                    String uomId=cartItem.getQuantityUomId();
                    String uomIdTo=null;
                  
                    GenericValue product=null;
                    try {
                      product = (GenericValue) delegator.findByPrimaryKey("Product",UtilMisc.toMap("productId", cartItem.getProductId()));
                    } catch (GenericEntityException e1) {
                       // TODO Auto-generated catch block
                       e1.printStackTrace();
                    }
                    ...
                    if(product!=null)
                    */
                    if(product!=null && UtilValidate.isNotEmpty(uomId) )
                    //End Neogia Specific : add Uom and price management
                    {
                        //check a product must note have an quantityUom if he hasn't a default Uom
                        //Begin Neogia Specific : add Uom and price management
                       ...
                       */
                       if (product.get("stockUomId")==null)
                        ...
                        try {
                            Map requestUom=UtilMisc.toMap("uomIdFrom",uomId);
                            requestUom.put("productId", product.get("productId"));
                            requestUom.put("originalValue",new Double(groupQty));                   
                            Map resultUom = dispatcher.runSync("conversionToProductUom",requestUom);
    
                            if (ServiceUtil.isError(resultUom))
                            {
                                Debug.logInfo("ServiceUtil.isError(resultUom)",null);
                                Map msgEr=UtilMisc.toMap("uomId",uomId);
                                msgEr.put("uomIdTo",uomIdTo);
                                return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderErrorCouldNotFindConversion", msgEr, locale));
                            }
                            if(resultUom.get("convertedValue")!=null){
                                groupQty = ((Double)resultUom.get("convertedValue")).doubleValue();
                            }
                        }
...
//              Begin Neogia Specific : BUG1491476 mooved for performance
//              ShoppingCartItem cartItem = cart.findCartItem(itemInfo[0]);
				String orderItemSeqId = itemInfo[0];
				String shipGroupSeqId = itemInfo[1];				
                //Begin Neogia Specific : Fix edit of Orderitem with OSISGA
                if(cart.isUpdate()  ){
                    //get OISGA
                    OrderItemShipGroupAssoc OISGA = ShoppingCartHelper.getOISGA(cart, orderItemSeqId, shipGroupSeqId);
                    
                    if( OISGA == null){
                        notFoundOISGAMap.put(orderItemSeqId + "_" + shipGroupSeqId, new Double(groupQty));
                    }
                    else {
                     //remove from  notFoundOISGAMap
                        notFoundOISGAMap.remove(orderItemSeqId + "_" +  shipGroupSeqId); 
                        
                    //set quantity to groupQty
                    OISGA.setQuantity(groupQty);
                    
                    //remove cancel quantity
                    OISGA.setCancelQuantity(0);
                    
                    //update display uom quantity
                    OISGA.setUpdateUomQuantity(true);

                    //update display uom cancel quantity
                    OISGA.setUpdateUomCancelQuantity(true);
                    ...
                    try {
                        ...
                        Debug.logError("Unable to store OISGA : " +  e.getLocalizedMessage(), module);
                        return ServiceUtil.returnError("OrderInternalError");
                    }
                    }
                    // Begin Neogia Specific - set item quantity in the right shipInfo
                    //if (orderItemSeqId.equals(itemOrderItemSeqId) && shipGroupSeqId.equals(shipInfoShipGroupSeqId)){
//                  update quantity temporary
                    cartItem.setQuantity(groupQty);
                    	int shipGroupIndex = cart.getShipInfoIndex(shipGroupSeqId);
                    	cart.setItemShipGroupQty(cartItem, groupQty, shipGroupIndex);
                    	Debug.logInfo("set in shipGroupIndex "+shipGroupIndex + " item " + cart.getItemIndex(cartItem) + " with quantity " + groupQty, module);
                    //}
                    // End Neogia Specific - set item quantity in the right shipInfo
                    // Begin Neogia Specific : FR#1706904 - Add fixedAsset usage information
            			if (reservStartMap.get(orderItemSeqId ) != null) {
            				String reservStartStr = reservStartMap.get(orderItemSeqId).toString();
            				Timestamp reservStart = null;
            				try {
            					reservStart = (Timestamp) ObjectType.simpleTypeConvert( reservStartStr, "java.sql.Timestamp", null, locale, true );
                            } catch (GeneralException e) {
                                // TODO Auto-generated catch block
                                e.printStackTrace();
                      // End Neogia Specific : FR#1706904 - Add fixedAsset usage info
                            }      
                            // Begin Neogia Specific: FR#1706904 - Asset usage info
            				cartItem.setReservStart(reservStart);
            				String reservLengthStr = reservLengthMap.get(orderItemSeqId).toString();
            				cartItem.setReservLength(UtilMisc.toDouble(reservLengthStr));
            				if (reservPersonsMap.get(orderItemSeqId) != null) {
            					String reservPersStr = reservPersonsMap.get(orderItemSeqId).toString();
            					cartItem.setReservPersons(UtilMisc.toDouble(reservPersStr));
            				}
            				try {
            					GenericValue workorderItemFulfill = EntityUtil.getFirst(delegator.findByAnd("WorkOrderItemFulfillment", UtilMisc.toMap("orderId", orderId, "orderItemSeqId" , orderItemSeqId)));
            					GenericValue workEffort = workorderItemFulfill.getRelatedOne("WorkEffort");
            					GenericValue fixedAsset = workEffort.getRelatedOne("FixedAsset");
            					//clear exception days
            					clearFixedAssetCalendar(delegator, dispatcher, userLogin, locale, workEffort, fixedAsset);
            					workEffort.set("estimatedStartDate",cartItem.getReservStart());
            					workEffort.set("estimatedCompletionDate",cartItem.getReservStart(cartItem.getReservLength()));
            					workEffort.set("reservPersons", new Double(cartItem.getReservPersons()));
            					workEffort.set("quantityToProduce", cartItem.getQuantity());
            					//update the calendar
            					updateFixedAssetCalendar(delegator, dispatcher, userLogin, locale, workEffort, fixedAsset, null, null, false);
            					delegator.store(workEffort);
            				} catch(GenericEntityException e) {
            					Debug.logError(e.getMessage(), module);
            				}
            				Debug.logInfo("reservation date is: " + reservStart, module);
            			}
                }
                // End Neogia Specific: FR#1706904 - Asset usage info
                else {
                	//we are never in this case
                	/*
                    if (orderItemSeqId.equals(itemOrderItemSeqId) && shipGroupSeqId.equals(shipInfoShipGroupSeqId)){
                        cart.setItemShipGroupQty(item, groupQty, index);
                        Debug.log("Set ship group qty: [" + itemInfo[0] + " / " + itemInfo[1] + " (" + (index) + ")] " + groupQty, module);
                    }
                    */
                }
//              End Neogia Specific : BUG1491476 mooved for performance
            }
            ...
            if (cartItem != null) {
                ...
                if (itemDescriptionMap != null && itemDescriptionMap.containsKey(itemSeqId)) {
                    ...
                    if (UtilValidate.isNotEmpty(description)) {
                        ...
                        //Begin Neogia SPecific : do not retrun error 
                        /*
                        return ServiceUtil.returnError("Item description must not be empty");
                        */
                        //End Neogia SPecific : do not retrun error 
                    }
                }
                
                // set estimated delivery date
                String itemDesiredDeliveryDate = null;
                if(itemDesiredDeliveryDateMap != null)
                itemDesiredDeliveryDate = (String) itemDesiredDeliveryDateMap.get(itemSeqId);
                Timestamp desiredDeliveryDate = null;
                
                try {
                    desiredDeliveryDate = (Timestamp) ObjectType.simpleTypeConvert( itemDesiredDeliveryDate, "java.sql.Timestamp", null, locale, true );
                } catch (GeneralException e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
                }               
                 cartItem.setDesiredDeliveryDate(desiredDeliveryDate);

                 // Begin Neogia Specific : edit orderItem comment
                 // set item comment
                 String itemComment = (String) itemCommentMap.get(itemSeqId);
                 cartItem.setItemComment(itemComment);
                 // End Neogia Specific : edit orderItem comment
        ...
        */
//      Begin Neogia Specific : BUG1491476 mooved for performance
        Iterator  mapIt = notFoundOISGAMap.entrySet().iterator();
        ...
        while(mapIt.hasNext()) {
           ...
           String[] OISGAInfo = key.split("_");
           String orderItemSeqId = OISGAInfo[0];
           String shipGroupSeqId = OISGAInfo[1];
           
           Map localContext = FastMap.newInstance();
           localContext.put("userLogin", userLogin);
           localContext.put("locale", locale);
           
           localContext.put("orderId", orderId);
           localContext.put("orderItemSeqId", orderItemSeqId);
           localContext.put("shipGroupSeqId", shipGroupSeqId);
           localContext.put("quantity", quantity);

           OrderItemShipGroupAssocServices.addOrderItemShipGroupAssoc(dctx, localContext);
        }
//      End Neogia Specific : BUG1491476 mooved for performance
    ...
    private static ShoppingCart loadCartForUpdate(LocalDispatcher dispatcher, GenericDelegator delegator, GenericValue userLogin, String orderId) throws GeneralException {
        ...
        if (shipGroupAssocs != null) {
            ...
            while (iri.hasNext()) {
                ...
                // Begin Neogia Specific : remove reservation only if orderItem is updated 
                GenericValue orderItem = (GenericValue) delegator.findByPrimaryKey("OrderItem",UtilMisc.toMap("orderId",orderId,"orderItemSeqId",orderItemSeqId));
                OrderItemShipGroup shipGroup = ((OrderItemShipGroupAssoc)shipGroupAssoc).getOrderItemShipGroup();
                if(!("ITEM_CREATED".equals(orderItem.getString("statusId")) ||"ITEM_APPROVED".equals(orderItem.getString("statusId")) || (shipGroup != null && OrderItemStatus.APPROVED.equals(shipGroup.getStatusEnumId())))) {
                    continue;
                }
                // End Neogia Specific : remove reservation only if orderItem is updated 
        ...
        ShoppingCartHelper.addOISGAList(cart, shipGroupAssocs);
        //End Neogia Specific : Fix edit of Orderitem with OSISGA
    ...
    private static void saveUpdatedCartToOrder(LocalDispatcher dispatcher, GenericDelegator delegator, ShoppingCart cart, Locale locale, GenericValue userLogin, String orderId) throws GeneralException {
        //Begin JIRA 518 
        Map cancelQuantityMap = new HashMap();
        ...
        //Begin  IWOOT specific : remove estimate shipping if free of shipping charge
        if(UtilValidate.isEmpty(cart.getFreeShippingProductPromoActions())){
        //End  IWOOT specific : remove estimate shipping if free of shipping charge
        ...
        //Begin  Neogia specific : remove estimate shipping if free of shipping charge
        }
        //End Neogia specific : remove estimate shipping if free of shipping charge
        ...
        //Begin Neogia Specific : add adjustement for shipping charge not associated to a specific ship group
        toStore.addAll(cart.makeAllOtherShippingCharges());
        //End Neogia Specific : add adjustement for shipping charge not associated to a specific ship group
        toStore.addAll(cart.makeAllOrderPaymentInfos(dispatcher));

//      Begin Neogia Specific : FR#1374361 set the new order item attributes 
        toStore.addAll(cart.makeAllOrderItemAttributes());
//      End Neogia Specific : FR#1374361
        ...
        while (tsi.hasNext()) {
            ...
            //Begin JIRA 518 
                ...
                GenericValue orderItem = delegator.findByPrimaryKey("OrderItem",UtilMisc.toMap("orderId",orderId,"orderItemSeqId",valueObj.getString("orderItemSeqId")));
                if (orderItem!= null){
                    //get cancel quantity and quantity
                    Double oldQuantity = (Double)orderItem.get("quantity");
                    Debug.logVerbose("oldQuantity : "+ oldQuantity,module);
                    if (oldQuantity == null)
                        oldQuantity = new Double(0.0);
                   Double quantity = (Double)valueObj.get("quantity");
                    Debug.logVerbose("quantity : "+ quantity,module);
                    if (quantity == null)
                        quantity = new Double(0.0);
                    
                    //set quantity
                    if(quantity.doubleValue() < oldQuantity.doubleValue() ){
                        valueObj.set("quantity",oldQuantity);
                        quantityMap.put(valueObj.getString("orderItemSeqId"),oldQuantity);
                    }
                    //set cancel quantity                
                    Double cancelQuantity = new Double(  oldQuantity.doubleValue() -quantity.doubleValue());
                    if(cancelQuantity.doubleValue()>0){
                        valueObj.set("cancelQuantity",cancelQuantity);
                        cancelQuantityMap.put(valueObj.getString("orderItemSeqId"),cancelQuantity);
                    }
                    else if(!"ITEM_CANCELLED".equals(valueObj.get("statusId")))
                    {
                        valueObj.set("cancelQuantity",new Double(0.0));
                        cancelQuantityMap.put(valueObj.getString("orderItemSeqId"),new Double(0.0));
                    }
                }
                // Begin Neogia Specific : FR1445562 
                // when orderitem is null
                else{
                    //begin Neogia SPecific : create a record into orderStatus for created status
                    //this is a new item                    
                    //if status is ITEM_APPROVED then first create a record in orderstatus for ITEM_CREATED
                    if(OrderItemStatus.APPROVED.equals(valueObj.get("statusId"))){
                        OrderStatus orderStatus = OrderStatusServices.makeValue(delegator, delegator.getNextSeqId("OrderStatus"));
                        orderStatus.put("orderItemSeqId", valueObj.get("orderItemSeqId"));
                        orderStatus.put("statusId", OrderItemStatus.CREATED);
                        orderStatus.put("orderId", orderId);
                        orderStatus.put("statusDatetime", UtilDateTime.nowTimestamp());
                        orderStatus.put("statusUserLogin", userLogin.getString("userLoginId"));
                        orderStatus.create();
                    }
                    //End Neogia SPecific : create a record into orderStatus for created status
                	 // update the orderStatus table when adding order item to an order
                     GenericValue orderStatus = delegator.makeValue("OrderStatus", null);
                     orderStatus.put("orderStatusId", delegator.getNextSeqId("OrderStatus"));
                     orderStatus.put("orderItemSeqId", valueObj.get("orderItemSeqId"));
                     orderStatus.put("statusId", valueObj.get("statusId"));
                     orderStatus.put("orderId", orderId);
                     orderStatus.put("statusDatetime", UtilDateTime.nowTimestamp());
                     orderStatus.put("statusUserLogin", userLogin.getString("userLoginId"));
                     orderStatus.create();
            ...
            //Begin JIRA 518 
                ...
                // Begin Neogia Specific : BUG1491476 
            	/*
            	String orderItemSeqId =  (String)valueObj.get("orderItemSeqId");

                Double cancelQuantity = (Double)cancelQuantityMap.get(orderItemSeqId);
                if(cancelQuantity != null && cancelQuantity.doubleValue()>0)
                    valueObj.set("cancelQuantity",cancelQuantity);
                else 
                    valueObj.set("cancelQuantity",new Double(0.0));
                    
                Double quantity = (Double)quantityMap.get(orderItemSeqId);
                if(quantity != null )
                    valueObj.set("quantity",quantity);
                */
                valueObj.set("cancelQuantity",new Double(0.0));
                // End Neogia Specific : BUG1491476
            }
//End JIRA 518 
        ...
        //Begin Neogia Specific : add OISGA for new orderItem
        //create a new OISGA for all new orderItem
        if(cart.isUpdate()){
            OrderHeader orderHeader = OrderHeaderServices.findByPrimaryKey(delegator, orderId);
            int shipGroupIndex = cart.getShipGroupSize() ;
            CartShipInfo csi = cart.getShipInfo(shipGroupIndex - 1 );
            ...
            try {
                OrderItemShipGroupAssocWorker.createMissingOISGAForOrderHeader(delegator,csi.shipGroupSeqId, orderHeader);
            }
            catch (GenericEntityException e ){
                Debug.logError("Error While creating missing OISGA fro orderHeader : " + e.getLocalizedMessage(), module);
                throw new GeneralException(e.getMessage());
            }
        }
        //End Neogia Specific : add OISGA for new orderItem        
        ...
        try {
            //Begin Neogia Specific : Fix edit of Orderitem with OSISGA
            orderItemShipGroupAssoc = delegator.findByAnd("OrderItemShipGroupAssoc", UtilMisc.toMap("orderId", orderId));
        } catch (GenericEntityException e) {
            Debug.logError(e, module);
            throw new GeneralException(e.getMessage());
        }
    ...
    // Begin Neogia specific
    /** Service to complete purchase order
     *  remove all OSEP associated with order
     *  change all OISG status to complete, this action will set status of OrderItems and orderHeader to completed
     *  */
    public static Map completePurchaseOrder (DispatchContext dctx, Map context){
    	LocalDispatcher dispatcher = dctx.getDispatcher();
        GenericDelegator delegator = dctx.getDelegator();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        Locale locale = (Locale) context.get("locale");
        ...
        // test if orderHeader is purchase order
        if (orderHeader.isPurchaseOrder()){
        	
        	// test if orderItems are processing_receiving or canceled or received or completed
        	List orderItemsExprs = UtilMisc.toList(new EntityExpr("statusId", EntityOperator.EQUALS, "ITEM_CREATED"));
        	orderItemsExprs.add(new EntityExpr("statusId", EntityOperator.EQUALS, "ITEM_APPROVED"));
        	EntityCondition orderItemsCond = new EntityConditionList(orderItemsExprs, EntityOperator.OR);
        	
        	OrderReadHelper orh = new OrderReadHelper(delegator, orderId);
        	List notValidStatusItems = orh.getOrderItemsByCondition(orderItemsCond);
        	
        	if (UtilValidate.isEmpty(notValidStatusItems)){
        		
        		// remove all OSEP associated with orderHeader
        		List OSEPs = OrderStockEventPlannedQuery.findByAndOrderHeader(delegator, orderId);
        		
        		if (UtilValidate.isNotEmpty(OSEPs)){
        			Iterator OSEPsIterator = OSEPs.iterator();
        			OrderStockEventPlanned OSEP = null;
                    
        			while (OSEPsIterator.hasNext()){
        				OSEP = (OrderStockEventPlanned) OSEPsIterator.next();
        				context.put("orderStockEventPlanned", OSEP);
        				OrderStockEventPlannedServices.removeOrderStockEventPlanned(dctx, context);
        			}
        		}
        		
        		// update all OISG status to completed
        		List orderItems = orderHeader.getOrderItems();

        		if (orderItems != null && !orderItems.isEmpty()){
        			Iterator orderItemsIterator = orderItems.iterator();
        			OrderItem orderItem = null;
        			while (orderItemsIterator.hasNext()){
        				orderItem = (OrderItem) orderItemsIterator.next();
        				List OISGAs = orderItem.getOrderItemShipGroupAssocs();
        				
        				if (OISGAs != null && !OISGAs.isEmpty()){
        					Iterator OISGAsIterator = OISGAs.iterator();
        					OrderItemShipGroupAssoc OISGA = null;
        					while (OISGAsIterator.hasNext()){
        						OISGA = (OrderItemShipGroupAssoc) OISGAsIterator.next();
        						String shipGroupSeqId = OISGA.getShipGroupSeqId();
        						
        						OrderItemShipGroup OISG = OrderItemShipGroupServices.findByPrimaryKey(delegator, orderId, shipGroupSeqId);
        						String OISGStatusId = OISG.getStatusEnumId();
        						
        						// change OISG status to completed
        						if (!OISGStatusId.equals(OrderItemStatus.COMPLETED)){
        							try{
        								Map ctxOISG = UtilMisc.toMap("orderId", orderId, "shipGroupSeqId", shipGroupSeqId, "newStatusId",  OrderItemStatus.COMPLETED, "userLogin", userLogin, "locale", locale);
        								dispatcher.runSync("changeOISGStatus",ctxOISG); 
        							}catch(GenericServiceException e){
        								return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderErrorCannotStoreStatusChanges", locale) +e.getMessage());
        							}
        		...
        		return ServiceUtil.returnError(UtilProperties.getMessage(resource, "OrderErrorCannotCompletePurchaseOrder", locale));
        	}
        }
        else
        	return ServiceUtil.returnError(UtilProperties.getMessage(resource, "OrderErrorCannotCompletePurchaseOrder", locale));
    ...
    //Begin Neogia specific: FR#1706904 - MAAZZI refactor fixed asset usage calendar exceptions
    public static void clearFixedAssetCalendar(GenericDelegator delegator, LocalDispatcher dispatcher, GenericValue userLogin, Locale locale, GenericValue workEffort, GenericValue fixedAsset) {
    	Timestamp estimatedStartDate = workEffort.getTimestamp("estimatedStartDate");
        Timestamp estimatedCompletionDate = workEffort.getTimestamp("estimatedCompletionDate");
        long dayCount = (estimatedCompletionDate.getTime() - estimatedStartDate.getTime())/86400000;
        while (--dayCount >= 0)    {
                ...
                */    
	            ...
	            if (techDataCalendarExcDay != null)    {
	                ...
	                if (newUsedCapacity > 0) {
	                	techDataCalendarExcDay.set("usedCapacity", newUsedCapacity);
	                	delegator.store(techDataCalendarExcDay);
	                } else {
	                	delegator.removeValue(techDataCalendarExcDay);
	                }
    ...
    public static void updateFixedAssetCalendar(GenericDelegator delegator, LocalDispatcher dispatcher, GenericValue userLogin, Locale locale, GenericValue workEffort, GenericValue fixedAsset, List toBeStored, List errorMessages, boolean creation) {
    	Timestamp estimatedStartDate = workEffort.getTimestamp("estimatedStartDate");
        Timestamp estimatedCompletionDate = workEffort.getTimestamp("estimatedCompletionDate");
        long dayCount = (estimatedCompletionDate.getTime() - estimatedStartDate.getTime())/86400000;
        while (--dayCount >= 0)    {
...
//                        " Requested: " + workEffort.getString("quantityToProduce"), module);
            ...
            if (creation) {
            	...
            	try {
            		delegator.createOrStore(techDataCalendarExcDay);
            	} catch (GenericEntityException e) {
                    Debug.logInfo(" techData excday could not be stored........", module);
                }
            }
//          Debug.logInfo("Update success CalendarID: " + techDataCalendarExcDay.get("calendarId").toString() +
//                    " and for date: " + techDataCalendarExcDay.get("exceptionDateStartTime").toString() +
//                    " and for quantity: " + techDataCalendarExcDay.getDouble("usedCapacity").toString(), module);
    ...
    // END Neogia specific: FR#1706904 - refactor fixed asset usage calendar update
    /**
