...
import org.ofbiz.order.orderReturn.developed.ReturnHeader;
...
public class OrderReturnServices {
    ...
    // Begin Neogia Specific: FR#1706904 - get initial reservLength for ASSET_USAGE
    public static Double getRentalInitialLength(GenericValue orderItem) {
    	GenericDelegator delegator = orderItem.getDelegator();
        BigDecimal rentalDurationAdj = BigDecimal.valueOf(1);
        try {
            List WorkOrderItemFulfillments = delegator.findByAndCache("WorkOrderItemFulfillment", UtilMisc.toMap("orderId", orderItem.getString("orderId"), "orderItemSeqId", orderItem.getString("orderItemSeqId")));
        	if (WorkOrderItemFulfillments != null && WorkOrderItemFulfillments.size() > 0) {
        		GenericValue workOrderItemFulfillment = EntityUtil.getFirst(WorkOrderItemFulfillments);
        		if( workOrderItemFulfillment != null ){
        			GenericValue workEffort = null;
        			workEffort = workOrderItemFulfillment.getRelatedOneCache("WorkEffort");
        			rentalDurationAdj = rentalDurationAdj.multiply(OrderReadHelper.getWorkEffortRentalQuantityBd(workEffort));
        			Debug.log("orderId " + orderItem.getString("orderId") + " / " + orderItem.getString("orderItemSeqId") + " RentalDurationAdj : " + rentalDurationAdj.doubleValue());
        		}
       	}
       	} catch(GenericEntityException e) {
       		Debug.log("Error reading the WorkOrderItemFulfillment: " + e.getMessage());
       	}
        return rentalDurationAdj.doubleValue();
    }
    // End Neogia Specific: FR#1706904 - get initial reservLength for ASSET_USAGE
    // Begin Neogia Specific: FR#1706904 - Calculate the asset usage length according to ReturnHeader entryDate and make rental specific adjustments.
    public static BigDecimal getRentalFinalLength(GenericValue orderItem, String returnId){
    	GenericDelegator delegator = orderItem.getDelegator();
        // get the return header
        GenericValue returnHeader = null;
        try {
        ...
        String entryDateStr = returnHeader.get("entryDate").toString().substring(0,10);
        entryDateStr += " 00:00:00.000000000";
        Timestamp returnEntryDate = java.sql.Timestamp.valueOf(entryDateStr);
        
        BigDecimal finalRentalDurationAdj = BigDecimal.valueOf(1);
        try {
            List WorkOrderItemFulfillments = delegator.findByAndCache("WorkOrderItemFulfillment", UtilMisc.toMap("orderId", orderItem.getString("orderId"), "orderItemSeqId", orderItem.getString("orderItemSeqId")));
        	if (WorkOrderItemFulfillments != null && WorkOrderItemFulfillments.size() > 0) {
        		GenericValue workOrderItemFulfillment = EntityUtil.getFirst(WorkOrderItemFulfillments);
        		if( workOrderItemFulfillment != null ){
        	        ...
        	        long length = 1;
        	        if (workEffort.get("estimatedStartDate") != null && returnEntryDate != null) {
        	        	if (workEffort.getTimestamp("estimatedStartDate").after(returnEntryDate)) {
        	        		Debug.logError("Start date can not be after return date", module);
        	        		return BigDecimal.valueOf(-1);
        	        		//throw new IllegalArgumentException("Return Date can't prior to Start date");
        	        	}
        	        	length = (returnEntryDate.getTime() - workEffort.getTimestamp("estimatedStartDate").getTime()) / 86400000;
        	        	if  (length == 0) {
        	        		length = 1;  // returned the same day means 1 day
        	        	}
        	        }
        	        ...
        	        if (persons.compareTo(new BigDecimal(1)) == 1)    {
        	            if (persons.compareTo(new BigDecimal(2)) == 1) {
        	                ...
        	                if(nthPersonPerc.signum() == 1)
        	                	finalRentalDurationAdj = persons.multiply(nthPersonPerc);
        	                else
        	                	finalRentalDurationAdj = persons.multiply(secondPersonPerc);
        	                persons = new BigDecimal("2");
        	            }
        	            if (persons.compareTo(new BigDecimal("2")) == 0)
        	            	finalRentalDurationAdj = finalRentalDurationAdj.add(secondPersonPerc);
        	            
        	            finalRentalDurationAdj = finalRentalDurationAdj.add(new BigDecimal(100));  // add final 100 percent for first person
        	            finalRentalDurationAdj = finalRentalDurationAdj.divide(new BigDecimal(100)).setScale(decimals, rounding).multiply(new BigDecimal(String.valueOf(length)));
        	            Debug.logInfo("Return rental info: Nbr of uom :" + persons + " 2nd uom %:" + secondPersonPerc + " Nth uom % :" + nthPersonPerc + " FinalLength: " + length + "  Final Total rental adjustment:" + finalRentalDurationAdj ,module);
        	        } else {
        	        	finalRentalDurationAdj = new BigDecimal(String.valueOf(length));
        	        }
        		}
       	}
       	} catch(GenericEntityException e) {
        ...
        return finalRentalDurationAdj; // return Final rental adjustment
        }

    // End Neogia Specific: FR#1706904 - Calculate the asset usage length according to ReturnHeader entryDate and make rental specific adjustments.
    ...
    public static Map getReturnableItems(DispatchContext dctx, Map context) {
        ...
        // Begin Neogia Specific: FR#1706904 - used to test ASSET_USAGE product at return
        boolean containAsset = false;
        // End Neogia Specific: FR#1706904 - used to test ASSET_USAGE product at return
        if (orderHeader != null) {
           ...
           Debug.log("orderItemQuantitiesIssued :" + orderItemQuantitiesIssued);
            // End Neogia specific
            } catch (GenericEntityException e) {
                Debug.logError(e, module);
                return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderErrorUnableToGetReturnHeaderFromItem", locale));
                ...
                while (i.hasNext()) {
                    ...
                    if (serviceResult.containsKey(ModelService.ERROR_MESSAGE)) {
                        ...
                        if (product != null) {
                            ...
                            // Begin Neogia Specific: FR#1706904: make ASSET_USAGE products shippable items
                        	if (!itemTypeKey.equals("FINISHED_GOOD")) {
                        		itemTypeKey = item.getString("orderItemTypeId");
                        		containAsset = true; // used to control RENTAL_ORDER_ITEM at return.
                        	}
                            // End Neogia Specific: FR#1706904: make ASSET_USAGE products shippable items
        ...
        // Begin Neogia Specific: FR#1706904: used to track ASSET_USAGE product at return
        result.put("returnAsset", containAsset);
        // End Neogia Specific: FR#1706904: used to track ASSET_USAGE product at return
        return result;
    }


    //  Begin Neogia Specific: FR#1706904 - update Fixed asset calendar
    public static boolean updateFixedAssetCalendarData(DispatchContext dctx, Map context, GenericValue returnItem) {
    	GenericDelegator delegator = dctx.getDelegator();
    	String returnItemTypeId = (String) returnItem.getString("returnItemTypeId");
    	String orderId = (String) returnItem.getString("orderId");
    	String orderItemSeqId = (String) returnItem.getString("orderItemSeqId");
        Double finalRentalLength = (Double) returnItem.getDouble("finalRentalLength");
        BigDecimal percentage = (new BigDecimal("0.01")).setScale(decimals, rounding);
        int taxCalcScale = UtilNumber.getBigDecimalScale("salestax.calc.decimals");
        int taxRounding = UtilNumber.getBigDecimalRoundingMode("salestax.rounding");
        
        if ( (returnItemTypeId != null) && (returnItemTypeId.equals("RET_END_FAPROD_ITEM")) ) {
        	try {
        		LocalDispatcher dispatcher = dctx.getDispatcher();
                GenericValue userLogin = (GenericValue) context.get("userLogin");
                Locale locale = (Locale) context.get("locale");
				...
				// clear related CalendarException data before creating a new one 
				org.ofbiz.order.order.OrderServices.clearFixedAssetCalendar(delegator, dispatcher, userLogin, locale, workEffort, fixedAsset);
				
				// prepare the new data and update
		        Timestamp estimatedStartDate = workEffort.getTimestamp("estimatedStartDate");
		        Timestamp estimatedCompletionDate = UtilDateTime.addDay(new Timestamp(estimatedStartDate.getTime()),(int)finalRentalLength.intValue());
				workEffort.set("estimatedCompletionDate",estimatedCompletionDate);
				// update fixedAsset calendar
				org.ofbiz.order.order.OrderServices.updateFixedAssetCalendar(delegator, dispatcher, userLogin, locale, workEffort, fixedAsset, null, null, false);
				delegator.store(workEffort);
				
                // now update order sales tax and orderHeader (we use returnQuantity and price since they are the same as in orderItem
				List OrderTaxAdjustments = delegator.findByAnd("OrderAdjustment", UtilMisc.toMap("orderId", orderId, "orderItemSeqId", orderItemSeqId, "orderAdjustmentTypeId", "SALES_TAX"));
				if (OrderTaxAdjustments != null && OrderTaxAdjustments.size() > 0){ 
            		Iterator orderAdj = OrderTaxAdjustments.iterator();
                	while (orderAdj.hasNext()) {
                		GenericValue OrderTaxAdjustment = (GenericValue) orderAdj.next();
                		BigDecimal percent = new BigDecimal(1);
                		if (OrderTaxAdjustment.get("sourcePercentage") != null)
                            percent = OrderTaxAdjustment.getBigDecimal("sourcePercentage").setScale(taxCalcScale,taxRounding);
                		OrderTaxAdjustment.set("amount", returnItem.getBigDecimal("returnQuantity").multiply(returnItem.getBigDecimal("returnPrice")).multiply(new BigDecimal(finalRentalLength)).multiply(percent).multiply(percentage).setScale(taxCalcScale, taxRounding));
                		OrderTaxAdjustment.store();
                	}
            	}
				return true;
        	} catch(GenericEntityException e) {
    ...
    //  End Neogia Specific: FR#1706904 - update Fixed asset calendar
    //  Begin Neogia Specific: FR#1706904 - Create rental invoice
    public static Map createFixedAssetAndAssociatedItemsInvoice(DispatchContext dctx, Map context, String orderId) {
    	GenericDelegator delegator = dctx.getDelegator();
    	Locale locale = (Locale) context.get("locale");
        // first get and update the order header
        GenericValue orderHeader = null;
        try {
            orderHeader = delegator.findByPrimaryKey("OrderHeader", UtilMisc.toMap("orderId", orderId));
        } catch (GenericEntityException e) {
    	...
    	try {
            List orderItems = delegator.findByAnd("OrderItem", UtilMisc.toMap("orderId", (String) orderId.toString()));
        ...
        return org.ofbiz.accounting.invoice.InvoiceServices.createInvoiceForOrder(dctx, context);
    }
    //  End Neogia Specific: FR#1706904 - Create rental invoice
    //  Begin Neogia Specific: FR1706904 - UPDATE returnItems status at receipt time.
    public static Map checkAndUpdateReturnItemsStatus(DispatchContext dctx, Map context) {
    	GenericDelegator delegator = dctx.getDelegator();
        String returnId = (String) context.get("returnId");
        Locale locale = (Locale) context.get("locale");
        ...
        try {
            returnHeader = delegator.findByPrimaryKey("ReturnHeader", UtilMisc.toMap("returnId", returnId));
            if (returnHeader != null && "RETURN_RECEIVED".equals(returnHeader.getString("statusId"))) {
                returnItems = returnHeader.getRelatedByAnd("ReturnItem", UtilMisc.toMap("statusId", "RETURN_REQUESTED"));
            }
        } catch (GenericEntityException e) {
        ...
        if (returnItems != null && returnItems.size() > 0) {
            ...
            for (Iterator itemsIter = returnItems.iterator(); itemsIter.hasNext(); ) {
                ...
                item.set("statusId", "RETURN_RECEIVED");
                toBeStored.add(item);
                Debug.logInfo("Switched statusId from RETURN_REQUESTED to RETURN_RECEIVED for returnItem with returnId/returnItemSeqId " + returnId + " / "+item.getString("returnItemSeqId"), module);
            }
            try {
    ...
    // End Neogia Specific: FR1706904 - UPDATE returnItems status at receipt time
    ...
    public static Map checkReturnComplete(DispatchContext dctx, Map context) {
       ...
       // Begin Neogia Specific: FR#1706904 - using this to check if we should create the rental invoice or not
       ...
       // Begin Neogia Specific: FR#1706904 - using this to check if we should create the rental invoice or not
        ...
        if (returnHeader != null && returnItems != null && returnItems.size() > 0) {
            ...
            while (itemsIter.hasNext()) {
                ...
                // Begin Neogia Specific: FR#1706904 - update the fixedAsset calendar only at RETURN_ACCPETED status
                String returnStatus = returnHeader.getString("statusId");
                if ( "RETURN_ACCEPTED".equals(returnStatus) && item != null ) {
                	String returnItemTypeId = (String) item.getString("returnItemTypeId");
                	if ( returnItemTypeId.equals("RET_END_FAPROD_ITEM")) { 
                		createInvoice = updateFixedAssetCalendarData(dctx, context, item);
                		if (!orderIds.contains(item.getString("orderId"))) {
                			orderIds.add((String) item.getString("orderId"));
                		}
                	}
                }
                // End Neogia Specific: FR#1706904 - update the fixedAsset calendar
                if (itemStatus != null) {
            ...
            // Begin Neogia Specific: FR#1706904 - create the rental invoice if the calendar has been updated
            if (createInvoice && (!orderIds.isEmpty() && orderIds.size() > 0 ) ) { 
            	Iterator orderIdIter = orderIds.iterator();
            	while (orderIdIter.hasNext()) {
            		String orderId = orderIdIter.next().toString();
            		context.put("orderId", orderId);
            		org.ofbiz.order.order.OrderServices.resetGrandTotal(dctx, context);
            		createFixedAssetAndAssociatedItemsInvoice(dctx, context, orderId);
            	}
            }
            // End Neogia Specific: FR#1706904 - create the rental invoice if the calendar has been updated
    ...
    public static Map processCreditReturn(DispatchContext dctx, Map context) {
        ...
        if (returnHeader != null && returnItems != null && returnItems.size() > 0) {
            ...
            for (Iterator itemsIter = returnItems.iterator(); itemsIter.hasNext(); ) {
                ...
                // Begin Neogia Specific: FR#1706904 - final rental length to adjust the creditTotal
                /*
                creditTotal = creditTotal.add(price.multiply(quantity).setScale(decimals, rounding));
                 */
                BigDecimal finalRentalLength = (!item.getBigDecimal("finalRentalLength").equals(ZERO)) ? item.getBigDecimal("finalRentalLength") : new BigDecimal(1);
                creditTotal = creditTotal.add((price.multiply(quantity).multiply(finalRentalLength)).setScale(decimals, rounding));
                // End Neogia Specific: FR#1706904 - final rental length to adjust the creditTotal
            }
    ...
    public static Map processRefundReturn(DispatchContext dctx, Map context) {
        ...
        */
        ...
        // End Neogia specific
            ...
            if( ! ((ReturnHeader) returnHeader).isRefundNeeded() )
               return ServiceUtil.returnSuccess();
                                    ...
                                    if (UtilValidate.isNotEmpty(repairItems)) {
                                        ...
                                        while (repairItemIt.hasNext()) {
                                            ...
                                            if (UtilValidate.isNotEmpty(repairItemProduct)) {
                                                ...
                                                //Begin Neogia SPecific : remove this comment after ofbiz revision  542814
                                                /*
                                                newItem.set("itemDescription", ProductContentWrapper.getProductContentAsText(repairItemProduct, "PRODUCT_NAME", locale, null));
                                                */
                                                //End Neogia SPecific : remove this comment after ofbiz revision  542814
                                                newItem.set("statusId", "ITEM_CREATED");
                                                orderItems.add(newItem);
    ...
    public static Map createReturnAdjustment(DispatchContext dctx, Map context) {
        ...
        if (returnItem != null) {  // returnAdjustment for returnItem
            if (needRecalculate(returnAdjustmentTypeId)) {
                ...
                // Begin Neogia Specific: FR#1706904 - use the final rental length values for salesTax adjustment
                /*
                ...
                */
                BigDecimal finalRentalLength = returnItem.getBigDecimal("finalRentalLength"); 
                if (!finalRentalLength.equals(ZERO) && "RET_SALES_TAX_ADJ".equals(returnAdjustmentTypeId) && orderAdjustment.getBigDecimal("sourcePercentage") != null) {
                	BigDecimal percentage = (new BigDecimal("0.01")).setScale(decimals, rounding);
                    int taxCalcScale = UtilNumber.getBigDecimalScale("salestax.calc.decimals");
                    int taxRounding = UtilNumber.getBigDecimalRoundingMode("salestax.rounding");
                    BigDecimal percent = orderAdjustment.getBigDecimal("sourcePercentage").setScale(taxCalcScale,taxRounding);
                	amount = (returnItem.getBigDecimal("returnQuantity").multiply(returnItem.getBigDecimal("returnPrice")).multiply(finalRentalLength).multiply(percent).multiply(percentage).setScale(taxCalcScale, taxRounding)).doubleValue();
                } else {
                	BigDecimal returnTotal = returnItem.getBigDecimal("returnPrice").multiply(returnItem.getBigDecimal("returnQuantity"));
                	BigDecimal orderTotal = orderItem.getBigDecimal("quantity").multiply(orderItem.getBigDecimal("unitPrice"));
                	amount = getAdjustmentAmount("RET_SALES_TAX_ADJ".equals(returnAdjustmentType), returnTotal, orderTotal, orderAdjustment.getBigDecimal("amount"));
                }
                // End Neogia Specific: FR#1706904 - use the final rental length values for salesTax adjustment
            } else {
        ...
        try {
            ...
            if (returnItem != null) {  // returnAdjustment for returnItem
                ...
                if (needRecalculate(returnAdjustmentTypeId)) {
                    ...
                    */
                	BigDecimal finalRentalLength = returnItem.getBigDecimal("finalRentalLength"); 
                    if (!finalRentalLength.equals(ZERO) && "RET_SALES_TAX_ADJ".equals(returnAdjustmentTypeId) && returnAdjustment.getBigDecimal("sourcePercentage") != null) {
                    	BigDecimal percentage = (new BigDecimal("0.01")).setScale(decimals, rounding);
                        int taxCalcScale = UtilNumber.getBigDecimalScale("salestax.calc.decimals");
                        int taxRounding = UtilNumber.getBigDecimalRoundingMode("salestax.rounding");
                        BigDecimal percent = returnAdjustment.getBigDecimal("sourcePercentage").setScale(taxCalcScale,taxRounding);
                    	amount = (returnItem.getBigDecimal("returnQuantity").multiply(returnItem.getBigDecimal("returnPrice")).multiply(finalRentalLength).multiply(percent).multiply(percentage).setScale(taxCalcScale, taxRounding)).doubleValue();
                    } else {
                    	BigDecimal returnTotal = returnItem.getBigDecimal("returnPrice").multiply(returnItem.getBigDecimal("returnQuantity"));
                        BigDecimal originalReturnTotal = new BigDecimal(originalReturnPrice).multiply(new BigDecimal(originalReturnQuantity));
