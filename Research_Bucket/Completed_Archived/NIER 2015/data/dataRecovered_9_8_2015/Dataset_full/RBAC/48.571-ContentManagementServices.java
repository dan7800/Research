...
Debug.logInfo("updateSiteRoles, serviceContext(2):" + serviceContext, module);
    ...
    public static Map persistDataResourceAndDataMethod(DispatchContext dctx, Map context) throws GenericServiceException, GenericEntityException, Exception {
      ...
      // Begin Neogia specific FR1352066
      /*
      ByteWrapper byteWrapper = (ByteWrapper)context.get("imageData");
      */
      ByteWrapper byteWrapper;
      boolean imgProcessing; 
      if ((ByteWrapper)context.get("imageData")!=null){
    	  byteWrapper = (ByteWrapper)context.get("imageData");
    	  imgProcessing = true;
      } else {
    	  byteWrapper = (ByteWrapper)context.get("fileData");
    	  imgProcessing = false;
      }
      // End of Neogia specific FR1352066
      ...
      */
      if (byteWrapper != null) {
    	  if (imgProcessing){
              ...
              mimeTypeId = (String) context.get("_fileData_contentType");
              String fileName = (String) context.get("_fileData_fileName");
              newDrContext.put("objectInfo", fileName);
              newDrContext.put("mimeTypeId", mimeTypeId);    		  
    	  }
          // End of Neogia specific FR1352066
      ...
      if ( dataResourceTypeId.indexOf("_FILE") >=0) {
          ...
          // Begin Neogia specific FR1352066
      } else if (dataResourceTypeId.equals("OTHER_OBJECT")){
    	  if (Debug.infoOn()) Debug.logInfo("create data resource for="+dataResourceTypeId,module);
          if (byteWrapper != null) {
              fileContext.put("dataResourceId", dataResourceId);
              fileContext.put("fileData", byteWrapper);
              fileContext.put("objectInfo",dataResource.get("objectInfo"));
              fileContext.put("mimeTypeId",mimeTypeId);
              thisResult = dispatcher.runSync("createFileDataResource", fileContext);
              errorMsg = ServiceUtil.getErrorMessage(thisResult);
              if (UtilValidate.isNotEmpty(errorMsg)) {
                  return ServiceUtil.returnError(errorMsg);
              }
          } else {
              //return ServiceUtil.returnError("'byteWrapper' empty when trying to create database file.");
          }    	  
    	  // End of Neogia specific FR1352066
      ...
      if (dataResourceTypeId.indexOf("_FILE") >=0) {
          ...
          if (byteWrapper != null || "true".equalsIgnoreCase(forceElectronicText)) {
              ...
              fileContext.put("fileData", byteWrapper);
              fileContext.put("objectInfo",dataResource.get("objectInfo"));
          	  fileContext.put("mimeTypeId",dataResource.get("mimeTypeId"));
              thisResult = dispatcher.runSync("updateFileDataResource", fileContext);
              errorMsg = ServiceUtil.getErrorMessage(thisResult);
              if (UtilValidate.isNotEmpty(errorMsg)) {
                  return ServiceUtil.returnError(errorMsg);
              }
