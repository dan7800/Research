...
public class ContentServices {
    ...
    // Begin Neogia specific : Add a new service for I18n management
    /**
     * findAlternateLocaleContent : Finds the alternate locale for a contentId
     */
    public static Map findAlternateLocaleContent(DispatchContext dctx, Map context){
    	...
    	if (contentId == null){
    		return ServiceUtil.returnError("No contentId given : couldn't find alternate Locale for unkown Content");
    	}
    	String localeContent = ContentWorker.findAlternateLocaleContent(delegator,contentId,locale);
   		if (localeContent == null){
   			return ServiceUtil.returnError("Unable to find alternate Locale for contentId="+contentId+" and Locale="+locale.getDisplayName());
   		}
   		result.put("localeContent",localeContent);
    	return result;
    }
    // End of Neogia specific
    ...
    public static Map findRelatedContent(DispatchContext dctx, Map context) {
        ...
        Debug.logInfo("toFrom="+toFrom,module);
