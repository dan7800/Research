...
\newcommand{\andy}[1]{\textcolor{green}{{\it [Andy says: #1]}}}
...
%%% Removed this form the original abstract

%In this paper, we describe the course, its objectives, activities and two projects. When describing the course, we provide a weekly breakdown of topics and activities which the instructor may follow. We also introduce two projects for the course, one which has students repair vulnerabilities in existing open source applications, and a second where students create a secure, small mobile application from scratch using proper engineering principles. We also discuss the Vulnerability of the Day activity, where students are exposed to real world vulnerabilities in mobile computing. The goal of the activity is to acclimate students with relevant, real world mobile vulnerabilities, demonstrate the importance of creating secure mobile applications, and to keep the student interest in the course high.
...
%While this course is the first known of its kind to combine security, mobile development and software engineering principles, there are numerous courses which individually focus on these areas.  
...
% *******
%	Check the spacing in some of the citations
