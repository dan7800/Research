...
% An Empirical Analysis of Crash Dump Grouping: How to Effectively Group Crash Dumps
% An Empirical Analysis of Mozilla Crash Dump Grouping
\title{An Empirical Analysis of Crash Dump Grouping: How to Effectively Group Crash Dumps}
...
% \keywords{Crash Dumps, Call Stacks, Grouping, Similarity} %% Remove this for space reasons
...
Due to the complexity of applications, it is impractical to expect perfect software on the initial release. Thus, an important and challenging task during software maintenance is to capture and diagnose failures triggered at the client side. Examples of such feedback systems include Microsoft's Dr. Watson~\cite{watson} and Mozilla Crash Reporter~\cite{mozilla}. In these systems, crash dumps which typically contain call stacks recorded at the crash site, are used for locating bugs in software. Since the same bug can be repeatedly triggered by different users and under different execution environments, the number of crash dumps submitted daily can reach millions~\cite{firefox}. Incorrectly grouping unrelated crash dumps or failing in identifying a new crash that actually belongs to an already fixed group can incur unacceptable manual overhead and potentially delay critical patches. Thus, it is imperative to find effective criteria for precisely correlating crash dumps that we can use to develop fast and automatic tools to assist with the crash grouping process.

Grouping crash dumps is challenging because the dumps primarily contain information which indicate dynamic symptoms of a bug, such as call stacks and register values. However, to locate a root cause and introduce a fix, we need to identify the part of the source code that is responsible for the crash. Since the same symptoms are potentially caused by different pieces of incorrect code or the same piece of problematic code can result in completely different call stacks at the crash, techniques purely based on dynamic symptoms for grouping crashes are insufficient~\cite{Bartz:2008:FSF:1855895.1855896,brodie:automated,4401026,Kim:2011:2}.
...
Using our dataset of a-groups and m-groups, our goal is to determine whether manual and automatic approaches apply consistent criteria and information to group crashes, and whether the two approaches correlate the same types of call stacks. As shown in Figure~\ref{fig:approach}, we compare the groups based on the four metrics, including:
\begin{itemize}
   \setlength{\itemsep}{.8pt} %Cut down on spacing for the different items in the list
    \setlength{\parskip}{0pt} %Cut down on spacing for the different items in the list
    \setlength{\parsep}{0pt}  %Cut down on spacing for the different items in the list
\item {\it Grouping criteria}: How can we group crash dumps and how can the groups be used?
...
&Localize bugs by comparing similar stacks\\\cline{2-2}
...
\multirow{2}{*}{Who Fixed the Bug}&Find experts to fix a group of bugs from\\
&the same code region or of same types\\\hline
\end{tabular}
%} % resize box
\end{table}


Interestingly, although the goal of grouping crash dumps is to avoid repeatedly diagnosing crash dumps caused by the same root cause, developers still frequently compare multiple crash dumps in the same group for determining the types and locations of a bug. This indicates that a group of crash dumps can be more informative than individual crash dumps in helping diagnose failures. For example, a bug may cause completely different symptoms, and we found a case where developers prioritize a group of crash dumps because one of the crashes have the same symptoms related to a security attack. As shown in the Table~\ref{tab:goal}, developers also correlate crash dumps if the root causes that lead to the crashes have a temporal or locality relationship. % \todo{find better example-see notes} % put back into mid paragraph


%%% Remove this. Unclear
In our study, we find cases where one crash is caused by an incomplete/incorrect fix to the other crashes. Developers believe that correlating these crashes can help quickly identify the cause and the fix for the newly generated crashes. The components where the crashes occur, together with the bug types, are used to determine the groups. The goal is to better distribute the debugging tasks to people who are most familiar with the code and the bug types.%\todo{remove paragraph. Confusing?- See review}
...
\item Diagnosing a group of crash dumps can be beneficial: first, a group of symptoms help determine general manifestation of a bug; second, a group of similar call stacks potentially enable automatic bug localizations. %\todo{talk about where this conclusion comes from}
\end{enumerate}
...
\scalebox{.9}{    %% Change always change this if size is a problem
...
In Figure~\ref{fig:ind} the $x$-axis, {\it Stack} represents call stacks. {\it Env} indicates the plugins and libraries involved in the crash. {\it Correlation}, used by Mozilla Crash Reporter, specifies how often a signature and a .dll component are simultaneously witnessed in a crash. For example, given a set of call stacks, we count that a certain signature occurs 10 times, and among the 5 times, a specific .dll also occurs; in this case, the correlation ratio between the signature and the component is 50\%. {\it Repro} and {\it URL} indicate two relevant types of testing information. {\it Repro} represents the steps taken to reproduce the bug and {\it URLs} refers to a list of URL visited before an application crashes. {\it Revision} is a list of changes made to the application shortly before a crash. Finally, {\it Build} gives the date and time to indicate when the application is built and installed, as well as the version of operating system where the application runs.
...
%%% Commented out 8/15
%In summary to our research question about the information used to group crashes, we found that:\todo{the 1st 3 of these just seem to be duplications of the facts - see comments}
%
%\begin{enumerate}
%   \setlength{\itemsep}{.8pt} %Cut down on spacing for the different items in the list
%    \setlength{\parskip}{0pt} %Cut down on spacing for the different items in the list
...
%%%% Placed here due to spacing

%\begin{table*}[!htb] % Not sure what these options do
\begin{table*}[ht]
%    \caption{Global caption}
\caption{Call Stack Similarity Fail to Distinguish Legitimate and Illegal Groups1~\label{tab:stack2}}
...
%Note:  6 pages + 2 pages at extra charge (max of 8 pages).

%%% Submission information

%https://www.softconf.com/h/sac2017/cgi-bin/scmd.cgi?scmd=aLogin&passcode=1009X-D7C3P3B7D7
